...
public:
    ...
    virtual cmp_t compare(index_t, index_t)
    {
        return CMP_EQ;
    };
};
...
bool refineWhen(T prototype, bool *cond)
{
    ...
    {
        if (partition->ranges[i] == 0 || partition->rounds[i] == partition->round)
        {
            continue;
        }
        if (!cond[i] || !prototype.perform(i, i + partition->ranges[i] - 1))
        {
            continue;
        }
        ...
        for (index_t j = i; j < i + partition->ranges[i]; j += partition->ranges[j])
        {
            cond[j] = true;
        }
    }
...
bool refineRange(T prototype, index_t from, index_t to)
{
    ...
    for (index_t i = from; i < to; i++)
    {
        ...
        if (partition->ranges[i] == 0 || partition->rounds[i] == partition->round || last > to)
        {
            continue;
        }
        if (prototype.perform(i, last))
        {
            emergence = true;
        }
    }
...
template <class T>
bool refineAll(T prototype)
{
    return refineRange<T>(prototype, 0, prototype.getPartition()->order - 1);
}

template <class T>
bool refinePlaces(T prototype)
{
    return refineRange<T>(prototype, 0, Net::Card[PL] - 1);
}

template <class T>
bool refineTransitions(T prototype)
{
    return refineRange<T>(prototype, Net::Card[PL], Net::Card[PL] + Net::Card[TR] - 1);
}
