...
void GeneratingSystem::create() {
    ...
    refinePlaces<UnboundedArrayRefinement<capacity_t> >(UnboundedArrayRefinement<capacity_t>(&partition, Marking::Initial));
	...
	partition.reset(); partitions[i] = partition.copy();
	partition.fix(i); refineFix(&partition);
    }
	...
	for(index_t i = 1; i < partition.order; i++) {
		...
		if(perm != NULL) { sigma[i] = perm; break; }
	    }
	}
    }
    
    // delete partitions
    for(index_t i = 0; i < partition.order; i++) delete partitions[i];
...
std::cout << end.tv_sec-start.tv_sec + (end.tv_usec-start.tv_usec)*1e-6 << " " << usage.ru_maxrss/1024.0 << std::endl;
