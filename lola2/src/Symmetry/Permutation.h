...
class Perm {
  ...
  public:
    ...
    T perm;
    T composeLeft(T permutation) { return perm.compose(permutation); }
    T composeRight(T permutation) { return permutation.compose(perm); }
    T inverse() { return perm.inverse(); }
    T power(int n) { return perm.power(n); }
    index_t stabilizes() { return perm.stabilizes(); };
    void show() { perm.show(); }
...
Perm<T>* Perm<T>::find(Partition *partition, index_t x, index_t y) {
    ...
    if(partition->representative[x] != partition->representative[y]) return NULL;
    
    // split codomain on x and domain on y
    Partition cod = *partition->copy(); cod.fix(x);
    Partition dom = *partition->copy(); dom.fix(y);
    if(!refineFix2(&cod, &dom)) { std::cout << "Inconsistent!" << std::endl; return NULL; }
    
    // search for symmetry
    cell_t *split = NULL;
    index_t *mapping = NULL;
    stack<index_t*> cods; cods.push(new index_t[partition->order]);
    memcpy(cods.top(), cod.ranges, sizeof(index_t) * partition->order);
    stack<index_t*> doms; doms.push(new index_t[partition->order]);
	...
	if(split == NULL) {
	    ...
	    for(index_t i = 0; i < partition->order; i++) mapping[cod.elements[i]] = dom.elements[i];
      
	    while(!candidates.empty()) { /*delete[] candidates.top();*/ candidates.pop(); }
	    while(!cods.empty()) { delete[] cods.top(); cods.pop(); }
	    while(!doms.empty()) { delete[] doms.top(); doms.pop(); }
	    break;
	}
	...
	while(!refineFix2(&cod, &dom)) {
		...
		delete[] cods.top(); cods.pop();
		delete[] doms.top(); doms.pop();
	    }
	    
	    if(candidates.empty()) return NULL; // dellocate properly
...
class Bijection {
  ...
  Bijection(index_t n, index_t *mapping) : size(n), map(mapping) {};
    ...
    static Bijection create(index_t n, index_t *mapping) { return Bijection(n, mapping); }
};
