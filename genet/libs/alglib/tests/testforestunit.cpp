...
static void basictest1(int nvars, int nclasses, int passcount, bool& err)
{
    ...
    for(pass = 1; pass <= passcount; pass++)
    {
        ...
        for(i = 0; i <= npoints-1; i++)
        {
            ...
            if( fabs(s-1)>1000*ap::machineepsilon )
            {
                err = true;
                return;
            }
            if( fabs(y(ap::round(xy(i,nvars)))-1)>1000*ap::machineepsilon )
            {
...
static void basictest3(bool& err)
{
    ...
    for(pass = 1; pass <= passcount; pass++)
    {
        ...
        for(i = 0; i <= npoints-1; i++)
        {
            ...
            if( ap::sqr(xy(i,0))+ap::sqr(xy(i,1))<=0.25 )
            {
                xy(i,2) = 0;
            }
            else
...
static void basictest5(bool& err)
{
    ...
    for(i = 0; i <= npoints-1; i++)
    {
        ap::vmove(&x(0), &xy(i, 0), ap::vlen(0,nvars-1));
        dfprocess(df, x, y);
        if( fabs(y(0)-xy(i,nvars))>1000*ap::machineepsilon )
        {
            eflag = true;
        }
    }
...
bool testforestunit_test_silent()
{
    ...
    return result;
#else
    return _i_testforestunit_test_silent();
#endif
}
