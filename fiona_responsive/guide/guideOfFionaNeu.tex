...
% Letzte Änderungen von: $Author: suermeli $ am $Date: 2008-03-31 12:13:11 $
...
\paragraph{Reduce IG}
Generate a reduced IG by applying reduction rules while the graph is
calculated. By default, the rules ``combine receiving events'' and ``receiving
before sending'' are applied. This can be changed by specifying the desired
rules as parameters. If any rule is given, only the specified rules are used.
...
\hline
\texttt{cre} & use the ``combine receiving events''-rule to reduce the IG \\
\hline
\texttt{rbs} & use the ``receiving before sending''-rule to reduce the IG \\
\hline
\end{tabular}
\end{quote}
...
\item Calculate the IG only using the ``combine receiving events''-rule, but
not the ``receiving before sending''-rule. 
\begin{quote}
\begin{verbatim}
fiona -t IG net.owfn -r -p cre
\end{verbatim}
\end{quote}
