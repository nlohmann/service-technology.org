...
class Block
{
    ...
    private:
	  ...
	  /// is true if the block is atomic and can be represented by a place, e.g. Startnode and Endnode
    bool atomic;
    ...
    /// Set of all input pinsets of the block
    set<string> inputCriteria;
    /// Set of all output pinsets of the block
    set<string> outputCriteria;

    /// Set of all inputBranches of the block (for control-flow gateways like merge)
    set<string> inputBranches;
    /// Set of all outputBranches of the block (for control-flow gateways like split)
    set<string> outputBranches;

    /// Set of additional Inputs of the block (when instantiating and extending task definitions)
    set<string> additionalInputs;
    /// Set of additional Outputs of the block (when instantiating and extending task definitions)
    ...
    void translateInclusiveDecisionPatternRecursively(Block* decision, Place* centralNode, set<string>& currentOutputBranches, string number, PetriNet* PN, BomProcess* bom, map<string,bool>& generated, set<set<string> >& powerSet);

    /// returns a prefix-string for node names at the global level
    const string globalPrefixString() const;

    /// statistical information: number of nodes of the process
    int num_nodes;
    /// statistical information: number of edges of the process
    int num_edges;
    /// statistical information: average number of input pins per node
    float avg_inDegree;
    /// statistical information: average number of output pins per node
    float avg_outDegree;

    /// number of input pins of the block
    int getInDegree ();
    /// number of output pins of the block
    int getOutDegree ();
    /// calculate statistical data about the static structure of the process
    void calculateStatisticalData();
  ...
  public:
    ...
    /// all blocks mentioned in the flow content of this block, only if this block is complex
    set<Block*> children;

    /// all connections of the current block, only if this block is complex
    ...
    /// creates a simple output string of this blocks structure for debugging
    ...
    /// return a CSV compatible string with statistical information about this process
    string statistics_toString(char separator=';');
};
