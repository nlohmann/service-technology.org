...
*/
void PNLoader::setInit(string s, int i) { initial[s]=i; }
...
*/
void PNLoader::setFinal(string s, int f) { required[s]=f; }

/** Set the mode for a place in the final marking.
	@param s The name of a place in the Petri net file.
	@param mode EQ (reach exactly), GE (cover), or LE (bound).
*/
void PNLoader::setCover(string s, int mode) { cover[s]=mode; }
...
*/
void PNLoader::setGeneralCover(bool cov) { generalcover = cov; }

/** Get the name of a problem.
	@return The name.
*/
string PNLoader::getName() { return name; }

/** Get the initial marking of a problem.
	@return The initial marking.
*/
Marking PNLoader::getInitialMarking() {
	...
	for(pit=pset.begin(); pit!=pset.end(); ++pit)
		if ((*pit)->getTokenCount()>0) result[(*pit)]=(*pit)->getTokenCount();
/*
...
*/
Marking PNLoader::getFinalMarking() {
	if (pn==NULL) getPetriNet();
	map<const Place*,unsigned int> result;
	map<string,int>::iterator it;
...
*/
void PNLoader::checkForNetReference(PNLoader& p) {
...
const vector<Place*>& PNLoader::getPOrder() { return placeorder; }

const map<Transition*,int>& PNLoader::getInvTOrder() { return revtorder; }

const map<Place*,int>& PNLoader::getInvPOrder() { return revporder; }
