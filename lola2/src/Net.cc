...
void print_net()
{
	...
	for(index_t i = 0; i < Net::Card[TR]; i++)
	{
		...
		{
		case NO_FAIRNESS: printf(" no "); break;
		case WEAK_FAIRNESS: printf(" weak "); break;
		case STRONG_FAIRNESS: printf(" strong "); break;
		...
		printf("\n conflicting:");
		for(index_t j = 0; j < Transition::CardConflicting[i];j++) printf("%s",Net::Name[TR][Transition::Conflicting[i][j]]);
		printf("\n");
		for(index_t j = 0; j < Transition::CardBackConflicting[i];j++) printf("%s",Net::Name[TR][Transition::BackConflicting[i][j]]);
