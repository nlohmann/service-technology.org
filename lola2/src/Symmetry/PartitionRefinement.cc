...
Partition::Partition(index_t o) : order(o), elements(NULL), representative(NULL), ranges(NULL), rounds(NULL), round(0), change(false) {
    if(order == 0) return;
    ...
    ranges = new index_t[order](); ranges[0] = order;
...
Partition *Partition::copy() {
    ...
    partition->round = round; partition->change = change;
    return partition;
}
...
void Partition::fix(index_t x) {
    assert(x >= 0 && x < order);
    index_t from = representative[x]; index_t to = from + ranges[from] - 1;
    if(from == to) return;
    
    index_t position; for(position = from; elements[position] != x; position++);
    exch(position, to);
    representative[x] = to;
    round++;
    ranges[from]--; rounds[from] = round;
    ranges[to] = 1; rounds[to] = round;
    change = true;
}

void Partition::reset() {
    change = false; round = 0;
...
inline void Refinement::confine(index_t left, index_t right) {
    ...
    if(partition->ranges[left] == 1 + right - left) return;
...
void PredicateRefinement::dispatch(index_t left, index_t right) {
    ...
    while(i <= j && j < (index_t)-1) if(evaluate(a[i])) partition->exch(i, j--); else i++;
    for(index_t k = left; k < i; k++) repr[a[k]] = left;
    for(index_t k = i; k <= right; k++) repr[a[k]] = i;
    
    if(i == left || j == right) { confine(left, right); return; }
    confine(left, i-1);
    confine(i, right);
}
...
void ComparisonRefinement::dispatch(index_t left, index_t right) {
    ...
    if(left == right) { repr[a[left]] = left; confine(left, right); return;  }
    if(right  < left || right == (index_t)-1) return;
    ...
    do {
	while(compare(a[j], v) == CMP_GT) j--;
	while(compare(a[i], v) == CMP_LT) if(++i > right) break;
    
	if(i == j) partition->exch(i++, p++);
	if(i  > j) break;
    
	if(compare(a[i], v) == CMP_EQ) partition->exch(i++, p++);
	partition->exch(i, j);
	if(compare(a[i], v) == CMP_EQ) partition->exch(i++, p++);
    ...
    if(i > p) for(index_t k = left, l = i-1; k < p; k++, l--) partition->exch(k, l);
    for(index_t k = left+i-p; k < i; k++) repr[a[k]] = left+i-p;
...
void CountingRefinement::counting(index_t left, index_t right, index_t low, index_t high) {
    ...
    if(right == left) { repr[a[left]] = left; confine(left, right); return; }
    if(right  < left || right == (index_t)-1) return;
    
    // count hits
    int hits[BUCKET_SIZE] = {0};
    for(index_t i = left; i <= right; i++) hits[value(a[i])-low]++;
    
    // calculate prefix sum and insert new cells
    int reprs[BUCKET_SIZE] = {0};
    int used = 0;
    if(hits[0] > 0) { confine(left, left+hits[0]-1); used++; }
    hits[0] += left; reprs[0] = left;
    for(int i = 1; i < BUCKET_SIZE; i++) {
	if(hits[i] > 0) { confine(hits[i-1], hits[i-1]+hits[i]-1); used++; }
	reprs[i] = hits[i-1]; hits[i] += hits[i-1];
    }
