...
*/
BFNode::BFNode() : p(""),cp(BFINVALID),op(0) {}
...
*/
bool BFNode::setLeaf(string place, int comp, int val) {
	if (cp==BFFORM) return false;
	p = place;
	cp = comp;
	op = val;
...
*/
string BFNode::getPlace() { return p; }
...
void BFNode::toDNF(bool neg) {
	if (cp!=BFFORM) { // negate atomic formulae
		if (neg) switch(cp) {
			...
			case BFEQ: 
				if (op>0) { // OR-split, Sara can't do inequalities
