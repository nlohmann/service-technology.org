     ...
     bool& err)
{
    ...
    for(pass = 1; pass <= passcount; pass++)
    {
        ...
        v = v/2;
        err = err||fabs((v-e)/v)>etol;
        err = err||fabs((mlperror(network, xy, 1)-v)/v)>etol;
        for(i = 0; i <= wcount-1; i++)
        {
            ...
            if( fabs(grad1(i))>1.0E-6 )
            {
                err = err||fabs((grad2(i)-grad1(i))/grad1(i))>etol;
            }
            else
            {
                err = err||fabs(grad2(i)-grad1(i))>etol;
            }
        }
        ...
        err = err||fabs((v-e)/v)>etol;
        err = err||fabs((mlperrorn(network, xy, 1)-v)/v)>etol;
        for(i = 0; i <= wcount-1; i++)
        {
            wprev = network.weights(i);
            network.weights(i) = wprev+h;
    ...
    for(pass = 1; pass <= passcount; pass++)
    {
        ...
        for(hkind = 0; hkind <= 1; hkind++)
        {
            ...
            for(i = 0; i <= wcount-1; i++)
            {
                for(j = 0; j <= wcount-1; j++)
                {
                    if( fabs(h1(i,j))>1.0E-6 )
                    {
                        err = err||fabs((h1(i,j)-h2(i,j))/h1(i,j))>etol;
                    }
                    else
                    {
                        err = err||fabs(h2(i,j)-h1(i,j))>etol;
                    }
                }
            }
        }
...
bool testmlpunit_test_silent()
{
    ...
    return result;
#else
    return _i_testmlpunit_test_silent();
#endif
}
