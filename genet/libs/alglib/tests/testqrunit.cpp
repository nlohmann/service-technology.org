...
static void testproblem(const ap::real_2d_array& a, int m, int n)
{
    ...
    for(i = 0; i <= m-1; i++)
    {
        for(j = 0; j <= m-1; j++)
        {
            ...
            if( i==j )
            {
                structerrors = structerrors||fabs(v-1)>=threshold;
            }
            else
            {
                structerrors = structerrors||fabs(v)>=threshold;
            }
        }
    }
    ...
    for(k = 1; k <= m-1; k++)
    {
        ...
        for(i = 0; i <= m-1; i++)
        {
            for(j = 0; j <= k-1; j++)
            {
                othererrors = othererrors||fabs(q2(i,j)-q(i,j))>10*ap::machineepsilon;
            }
        }
    }
}
...
bool testqrunit_test_silent()
{
    ...
    return result;
#else
    return _i_testqrunit_test_silent();
#endif
}
