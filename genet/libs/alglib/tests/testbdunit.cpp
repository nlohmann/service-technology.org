...
static void testproblem(const ap::real_2d_array& a, int m, int n)
{
    ...
    for(i = 0; i <= m-1; i++)
    {
        for(j = 0; j <= n-1; j++)
        {
            ...
            decomperrors = decomperrors||fabs(v-a(i,j))>threshold;
        }
    }
    ...
    for(i = 0; i <= m-1; i++)
    {
        for(j = 0; j <= m-1; j++)
        {
            ...
            if( i==j )
            {
                properrors = properrors||fabs(v-1)>threshold;
            }
            else
            {
    ...
    for(k = 1; k <= m-1; k++)
    {
        ...
        for(i = 0; i <= m-1; i++)
        {
            for(j = 0; j <= k-1; j++)
            {
                parterrors = parterrors||fabs(r(i,j)-q(i,j))>10*ap::machineepsilon;
            }
        }
    }
    for(k = 1; k <= n-1; k++)
    {
        ...
        for(i = 0; i <= k-1; i++)
        {
            for(j = 0; j <= n-1; j++)
            {
                parterrors = parterrors||r(i,j)-pt(i,j)!=0;
            }
        }
    }
    ...
    mulerrors = mulerrors||matrixdiff(r1, r2, mtsize, m)>threshold;
    makeacopyoldmem(x, mtsize, m, r);
    ...
    mulerrors = mulerrors||matrixdiff(r1, r2, m, mtsize)>threshold;
    makeacopyoldmem(x, m, mtsize, r);
    ...
    mulerrors = mulerrors||matrixdiff(r1, r2, mtsize, n)>threshold;
    makeacopyoldmem(x, mtsize, n, r);
    ...
    mulerrors = mulerrors||matrixdiff(r1, r2, n, mtsize)>threshold;
    makeacopyoldmem(x, n, mtsize, r);
...
bool testbdunit_test_silent()
{
    ...
    return result;
#else
    return _i_testbdunit_test_silent();
#endif
}
