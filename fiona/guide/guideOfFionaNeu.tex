...
% Letzte Änderungen von: $Author: weinberg $ am $Date: 2008-03-06 16:18:53 $
...
\texttt{PRODUCE} are allowed to be empty, thus representing a transition which is not in connected
to the rest of the net in any way.

\subsubsection{Initial Marking}
After the \texttt{PLACES} section comes the \texttt{INITIALMARKING} section. Every place which is
not listed in the \texttt{INITIALMARKING} section is implicitly assumed to have zero tokens
...
number of tokens of each place. \texttt{INITIALMARKING} section can look as follows:
\begin{quote}
\begin{verbatim}
INITIALMARKING
  p1:  2,
...
one, including not mentioned places as having a zero tokens. In the same way this means, that a
\texttt{FINALMARKING} can only describe exactly one final state. A final marking looks like this:
\begin{quote}
\begin{verbatim}
FINALMARKING
  p2:  4;
...
\textsc{Fiona} can have the og file format as its input.
