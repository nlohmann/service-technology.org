...
/// search an element into DNF structure
	...
	while (!lookupStack.empty() && !duplicated )
	{
		...
		while (*temp_iterator != *temp_set.end() )
		{
			...
			if (!superset && subset) {
				...
				//map<int, set<PartialOrder *> >::const_iterator poit = po_map.find( search_depth );
				if ( (search_depth - temp_depth) == 1 ) {
					...
					os << "push : "; temp_element->out(os) << endl;
...
DNF::DNF(Formula * f, set<string> & literals, std::ostream & os) :
	...
	// string str = "false";
	Literal * l = new Literal("false"); //& str);
	// l_clause of root node is always a "false" string
	set<string> l_clause;
	l_clause.insert("false"); //str);
...
DNF::~DNF(){
	...
	delete_DNF_structure( delete_nodes );  delete(po);  delete(originalFormula);
	po_map.clear();
	disjuncts.clear();
	all_literals.clear();
}
