...
bool testbdss(bool silent)
{
    ...
    for(n = 1; n <= maxn; n++)
    {
        ...
        if( n>1 )
        {
            ...
            split2errors = split2errors||fabs(threshold-0.5)>100*ap::machineepsilon;
            split2errors = split2errors||fabs(pal-1)>100*ap::machineepsilon;
            split2errors = split2errors||fabs(pbl-0)>100*ap::machineepsilon;
            split2errors = split2errors||fabs(par-0)>100*ap::machineepsilon;
            split2errors = split2errors||fabs(pbr-1)>100*ap::machineepsilon;
        }
    }
    ...
    {
        split2errors = split2errors||fabs(threshold-0.195)>100*ap::machineepsilon;
    ...
    for(n = 1; n <= maxn; n++)
    {
        ...
        if( n>1 )
        {
            ...
            split2errors = split2errors||fabs(threshold-0.5)>100*ap::machineepsilon;
            split2errors = split2errors||fabs(rms-0)>100*ap::machineepsilon;
            if( n==2 )
            {
                split2errors = split2errors||fabs(cvrms-0.5)>100*ap::machineepsilon;
            }
            else
            {
                if( n==3 )
                {
                    split2errors = split2errors||fabs(cvrms-sqrt((2*0+2*0+2*0.25)/6))>100*ap::machineepsilon;
                }
                else
                {
                    split2errors = split2errors||fabs(cvrms)>100*ap::machineepsilon;
                }
            }
        }
    }
    ...
    {
        split2errors = split2errors||fabs(threshold-(n-2.5))>100*ap::machineepsilon;
        split2errors = split2errors||fabs(rms-sqrt((0.25+0.25+0.25+0.25)/(3*n)))>100*ap::machineepsilon;
        split2errors = split2errors||fabs(cvrms-sqrt(double(1+1+1+1)/double(3*n)))>100*ap::machineepsilon;
    }
    ...
    for(n = 1; n <= maxnq; n++)
    {
        ...
        if( n>1 )
        {
            ...
            optimalsplitkerrors = optimalsplitkerrors||fabs(thresholds(0)-0.5)>100*ap::machineepsilon;
            optimalsplitkerrors = optimalsplitkerrors||fabs(cve-(-c0*log(double(c0)/double(c0+1))-c1*log(double(c1)/double(c1+1))))>100*ap::machineepsilon;
        }
        ...
        if( n>=16 )
        {
            ...
            if( ni==nc )
            {
                for(i = 0; i <= nc-2; i++)
                {
                    optimalsplitkerrors = optimalsplitkerrors||fabs(thresholds(i)-(c0*(i+1)-1+0.5))>100*ap::machineepsilon;
                }
                cvr = -((nc-1)*c0*log(double(c0)/double(c0+nc-1))+c1*log(double(c1)/double(c1+nc-1)));
                optimalsplitkerrors = optimalsplitkerrors||fabs(cve-cvr)>100*ap::machineepsilon;
            }
        }
    }
    ...
    for(n = 1; n <= maxnq; n++)
    {
        ...
        if( n>1 )
        {
            ...
            if( ni==2 )
            {
                splitkerrors = splitkerrors||fabs(thresholds(0)-0.5)>100*ap::machineepsilon;
                splitkerrors = splitkerrors||fabs(cve-(-c0*log(double(c0)/double(c0+1))-c1*log(double(c1)/double(c1+1))))>100*ap::machineepsilon;
            }
        }
        ...
        for(c0 = 4; c0 <= n; c0++)
        {
            if( n%c0==0&&n/c0<=c0&&n/c0>1 )
            {
                ...
                if( ni==nc )
                {
                    for(i = 0; i <= nc-2; i++)
                    {
                        splitkerrors = splitkerrors||fabs(thresholds(i)-(c0*(i+1)-1+0.5))>100*ap::machineepsilon;
                    }
                    cvr = -nc*c0*log(double(c0)/double(c0+nc-1));
                    splitkerrors = splitkerrors||fabs(cve-cvr)>100*ap::machineepsilon;
                }
            }
        }
    }
...
bool testbdssunit_test_silent()
{
    ...
    return result;
#else
    return _i_testbdssunit_test_silent();
#endif
}
