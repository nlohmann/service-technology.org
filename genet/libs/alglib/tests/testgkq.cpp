...
bool testgkqunit(bool silent)
{
    ...
    for(pkind = 0; pkind <= 5; pkind++)
    {
        ...
        for(i = 0; i <= n-1; i++)
        {
            vstblerrors = vstblerrors||fabs(x1(i)-x2(i))>errtol;
            vstblerrors = vstblerrors||fabs(wk1(i)-wk2(i))>errtol;
            vstblerrors = vstblerrors||fabs(wg1(i)-wg2(i))>errtol;
        }
    }
