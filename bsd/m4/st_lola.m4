...
AC_PATH_PROG(LOLA, [lola-boundedgraph], [])
if test "${LOLA}" = ""; then
  ...
  make lola-boundedgraph > /dev/null 2> /dev/null
  make install > /dev/null 2> /dev/null
  cd ..

  cp "$LOLADIR"local/bin/lola-boundedgraph* .
  rm -fr $LOLADIR lola.tar.gz

  AC_PATH_PROG(LOLA, [lola-boundedgraph], [])
else
AC_PATH_PROGS_FEATURE_CHECK([LOLAVERSION], [lola-boundedgraph], [[ac_cv_path_LOLAVERSION=`$ac_path_LOLAVERSION --version | ${SED} -e 's/LoLA //'`]])
