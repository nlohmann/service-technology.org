...
void setpersistentbit(binDecision* d) {
    if (d -> nextold) {
        setpersistentbit(d -> nextold);
    }
    if (d -> nextnew) {
        setpersistentbit(d -> nextnew);
    }
#ifdef SWEEP
    d -> persistent = 1;
#endif
    ...
    while (1) {
        ...
        while (CurrentSweep[CurrentSection * ProgressSpan + ProgressValueLow]) {
            ...
            for (currentfired = 0; fl[currentfired]; currentfired++) {
                ...
                if (NewPos >= ProgressSpan) {
                    NewPos -= ProgressSpan;
                }
                NewPos3 = ProgressValueLow + fl[currentfired] -> progress_value + CurrentSection * ProgressSpan;
                if (NewPos3 < 0) {
                    NewPos3 += 3 * ProgressSpan;
                }
                if (NewPos3 >= 3 * ProgressSpan) {
                    NewPos3 -= 3 * ProgressSpan;
                }
                // search
                if ((!(fl[currentfired]->progress_value)) &&  binSearch(SameValue)) {
                    goto back;
                }
                if ((!MonotoneProgress) && binSearch(OldPersistent[NewPos])) {
                    goto back;
                }
                if (binSearch(CurrentSweep[NewPos3])) {
                    goto back;
                }
...
// end check property
                ...
                checkMaximalStates(CurrentMemUse); ///// LINE ADDED BY NIELS
#endif
                if (CurrentMemUse > PeakMemUse) {
                    PeakMemUse = CurrentMemUse ;
                }
                BucketSize[NewPos3] ++;
                if (fl[currentfired] -> progress_value >= 0) {
                    NrAhead++;
                } else {
                    NrBehind ++;
                    NrPersistent++;
