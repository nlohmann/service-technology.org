    ...
    for(nf = 1; nf <= maxnf; nf++)
    {
        for(nc = 2; nc <= maxnc; nc++)
        {
            for(pass = 1; pass <= passcount; pass++)
            {
                ...
                ldanerrors = ldanerrors||wn(axis,0)<=0.90;
                ...
                if( nf>=3 )
                {
                    ...
                    ldanerrors = ldanerrors||!testwn(xy, wn, ns, nf, nc, 1);
                    ldanerrors = ldanerrors||sqrt(ap::sqr(wn(nf-1,nf-1))+ap::sqr(wn(nf-2,nf-1)))<=0.90;
                    ldanerrors = ldanerrors||fabs(wn(nf-1,nf-1)+wn(nf-2,nf-1))>=0.10;
                    fisherlda(xy, ns, nf, nc, info, w1);
                    for(i = 0; i <= nf-1; i++)
                    {
                        lda1errors = lda1errors||w1(i)!=wn(i,0);
     ...
     int ndeg)
{
    ...
    {
        result = result&&jp(i-1)/jq(i-1)>=(1-tol*ap::machineepsilon)*jp(i)/jq(i);
    }
    for(i = nf-1-ndeg+1; i <= nf-1; i++)
    {
        result = result&&jp(i)<=tol*ap::machineepsilon*jp(0);
    }
    ...
    for(i = 0; i <= nf-1; i++)
    {
        wprev = tx(i);
        tx(i) = wprev+0.01;
        result = result&&v>=(1-tol*ap::machineepsilon)*calcj(nf, st, sw, tx, p, q);
        tx(i) = wprev-0.01;
        result = result&&v>=(1-tol*ap::machineepsilon)*calcj(nf, st, sw, tx, p, q);
        tx(i) = wprev;
    }
    ...
    {
        result = result&&tx(0)>tx(nf-1)*1000*ap::machineepsilon;
    }
    ...
    for(j = 0; j <= nf-1; j++)
    {
        ...
        v = sqrt(v);
        result = result&&fabs(v-1)<=1000*ap::machineepsilon;
        v = 0;
        for(i = 0; i <= nf-1; i++)
        {
            v = v+wn(i,j);
...
bool testldaunit_test_silent()
{
    ...
    return result;
#else
    return _i_testldaunit_test_silent();
#endif
}
