...
LinkedList<split_cell_t> NeighbourhoodRefiner::gather(index_t i, index_t *repr)
{
    ...
    for (index_t j = 0; j < neighbours[i]->order; j += neighbours[i]->ranges[j])
    {
        list.enqueue(getInfo(i, j, repr));
    }
    return list;
}
...
bool NeighbourhoodRefiner::progress(bool *suspects)
{
    ...
    for (index_t i = 0; i < partition->order; i++)
    {
        ...
        for (list.reset(), cells[i].reset(); !change && list.isValid()
                && cells[i].isValid(); list.next(), cells[i].next())
        {
        ...
        {
            emergence = true;
            suspects[partition->representative[i]] = true;
        }
...
{
    return refineAll<SCCNeighbourRefinement>(SCCNeighbourRefinement(neighbours[i], scc, partition, i,
            direction));
}
...
void refineFix(Partition *partition)
{
    ...
    do
    {
        ...
        for (int dir = PRE; dir <= POST; dir++)
        {
            do
            {
                refineWhen<BoolArrayRefinement>(BoolArrayRefinement(partition, neighbourhood[dir]->available),
                                                suspects[dir]);
                ...
                refineWhen<BoundedArrayRefinement>(BoundedArrayRefinement(partition, neighbourhood[dir]->lengths),
                                                   suspects[dir]);
                refineWhen<BoundedArrayRefinement>(BoundedArrayRefinement(partition, neighbourhood[dir]->reprs),
                                                   suspects[dir]);
            }
            ...
            for (index_t i = 0; i < partition->order; i += partition->ranges[i])
            {
                suspects[dir][i] = false;
            }
            neighbourhood[dir]->progress(suspects[dir]);
        }

        if (!partition->change) for (index_t i = 0; i < partition->order; i += partition->ranges[i])
            {
                assert(partition->rounds[i] < partition->round);
            }
    }
    while (partition->change);

    delete neighbourhood[PRE];
    delete neighbourhood[POST];
    delete[] suspects[PRE];
    delete[] suspects[POST];
}
...
bool refineFix2(Partition *p1, Partition *p2)
{
    ...
    do
    {
        ...
        for (int dir = PRE; dir <= POST; dir++)
        {
            ...
            do
            {
                if (!isomorphic(neighbourhood[0][dir]->available, neighbourhood[1][dir]->available, order))
                {
                    return false;
                }
                if (!isomorphic(neighbourhood[0][dir]->lengths, neighbourhood[1][dir]->lengths, order))
                {
                    std::cout << "lengths\n";
                    return false;
                }
                ...
                for (int i = 0; i < 2; i++)
                {
                    refineWhen<BoolArrayRefinement>(BoolArrayRefinement(p[i], neighbourhood[i][dir]->available),
                                                    suspects[i][dir]);
                    ...
                    refineWhen<BoundedArrayRefinement>(BoundedArrayRefinement(p[i], neighbourhood[i][dir]->lengths),
                                                       suspects[i][dir]);
                    refineWhen<BoundedArrayRefinement>(BoundedArrayRefinement(p[i], neighbourhood[i][dir]->reprs),
                                                       suspects[i][dir]);
                }

                advance = neighbourhood[0][dir]->next();
                if (neighbourhood[1][dir]->next() != advance)
                {
                    return false;
                }
            }
    ...
    for (int i = 0; i < 2; i++)
    {
        delete neighbourhood[i][PRE];
        delete neighbourhood[i][POST];
    }
    return true;
}
