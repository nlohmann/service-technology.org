...
do
  ...
  printf "lola-%s:\n\t\$(AM_V_at)\$(MAKE) bin-lola-%s\$(EXEEXT) \$(AM_MAKEFLAGS) DEFAULT_INCLUDES=\"-I \$(srcdir)/configs/%s \$(DEFAULT_INCLUDES)\" CPPFLAGS=-DSTANDARDCONFIG\n" $FILE $FILE $FILE >> Makefile.configs
  printf "\t\$(AM_V_GEN)mv \$(builddir)/bin-lola-%s\$(EXEEXT) \$(builddir)/lola-%s\$(EXEEXT)\n\n" $FILE $FILE >> Makefile.configs
done
