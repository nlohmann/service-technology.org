...
IO::~IO()
{
    r->status("closed %s file %s", r->markup(MARKUP_OUTPUT, kind.c_str()).str(), r->markup(MARKUP_FILE,
              filename.c_str()).str());
    fclose(fp);
}
...
Output::Output(std::string kind) :
    IO(stdout, kind, "")
{
    r->status("writing %s to %s", r->markup(MARKUP_OUTPUT, kind.c_str()).str(), r->markup(MARKUP_FILE,
              "stdout").str());
}
...
Input::Input(std::string kind) :
    IO(stdin, kind, "")
{
    r->status("reading %s from %s", r->markup(MARKUP_OUTPUT, kind.c_str()).str(), r->markup(MARKUP_FILE,
              "stdin").str());
}
