...
returnTypes = { NetAnalysisInformation.BOUNDEDNESS.class, UnboundedPlacesSet.class, LoLADiagnosis.class }, //
userAccessible = true, //
help = "Calls LoLA to check whether a given Petri net is bounded.", mostSignificantResult = 3)
public class LoLA_Boundedness {
	// choose the respective LoLA binary
	private final LoLA_Executor lola = LoLA_Executor.BOUNDEDNET;
	...
	private Object[] checkBoundedness(PluginContext context, Petrinet net, Marking marking) throws Exception {
		...
		boundednessInfo.setValue(unbounded ? UnDetBool.FALSE : UnDetBool.TRUE);

		// extract unbounded places from results file
		UnboundedPlacesSet unboundedPlacesSet = new UnboundedPlacesSet();
		if (unbounded) {
			CTMarking omegaMarking = LoLA_Executor.constructMarking(net, (ConfigurationList) cfg.getValue(
					"unbounded.state", null));
		...
		context.addConnection(new BoundednessInfoConnection(net, marking, PetrinetSemanticsFactory
				.regularPetrinetSemantics(Petrinet.class), (NetAnalysisInformation.BOUNDEDNESS) result[0]));
		context.addConnection(new UnboundedPlacesConnection(net, (UnboundedPlacesSet) result[1], marking,
