...
//	pthread_attr_setstacksize(&attr,1000000);
	...
	for(unsigned int i=0; i<maxthreads; ++i)
	{
		int res = pthread_create(&(threads[i]),&attr,concurrentPathFinder,NULL);	
...
*/
void initThread(unsigned int rtnr, PartialSolution* ps, Marking& m, map<Transition*,int>& tv, PathFinder& pf) {
...
//	for(unsigned int i=0; i<=pf.tvsize; ++i)
//	{
//		threaddata[rtnr]->stubsets.push_back(vector<Transition*> ());
//		threaddata[rtnr]->stubsets.back().reserve(8);
//	}
...
//	threaddata[rtnr]->psets.reserve((pf.im.tMaxPreset()+pf.im.tMaxPostset())*2);
...
//	for(unsigned int i=help->stubsets.size(); i<=me->pf->tvsize; ++i)
//	{
//		help->stubsets.push_back(vector<Transition*> ());
//		help->stubsets.back().reserve(8);
//	}
...
//	help->psets.reserve((me->pf->im.tMaxPreset()+me->pf->im.tMaxPostset())*2);
...
/** Signal depending threads to restart.
	@param debug Falg for status messages.
*/
void makeThreadsIdle(int debug) {
	// this function needs main_mutex to be locked
...
//pthread_mutex_lock(&(print_mutex));
//cerr << "[" << i+1 << "] " << threaddata[i+1]->activity << " jobs done" << endl;
//pthread_mutex_unlock(&(print_mutex));
	...
	while (true) {
		somework=false;
		for(unsigned int i=0; i<maxthreads; ++i) if (!threaddata[i+1]->idle) somework=true;
		if (!somework) break;
		if (debug) {
			pthread_mutex_lock(&(print_mutex));
			cerr << "[M] Sleeping ..." << endl;
			pthread_mutex_unlock(&(print_mutex));
		}
