...
public class ModelEditor extends hub.top.editor.ModelEditor implements hub.top.editor.eclipse.IFrameWorkEditor {
  ...
  /**
   * Parse the given text into a P/T-net model.
   * 
   * @param text
   * @return
   *     <code>null</code> iff the text could not be parsed to a model 
   */
  public static PtNet getModel(String text) {
    EObject result = null;
    
    try {
      Syntax syntax = ModelEditor.createSyntax_default();
      
      IModelCreatingContext context = new LolaModelCreatingContext(
          ModelEditor.createMetaModelPackages_default(),
          new DefaultSemanticsProvider(DefaultIdentificationScheme.INSTANCE),
          new ResourceImpl(),
          text);
      
      result = ModelCreating.createModelFromText(context, syntax);
      
    } catch (ModelCreatingException e) {
      Activator.getPluginHelper().logError("Failed to create model.", e);
    }
    
    if (result == null || !(result instanceof PtNet))
      return null;
