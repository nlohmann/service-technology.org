...
DIRS=`find . | grep "configure.ac" | grep -v ".svn" | sed -e 's/configure.ac//g;s/\/\//\//;s/.\///'`
...
echo "AC_INIT([service-technology.org toolbox], r`svn info | grep "Revision" | awk '{print $2}'`, tools@service-technology.org, service-tech, http://service-technology.org)" > configure.ac
...
autoreconf -isv -Wnone
./configure --prefix=`pwd`/build --docdir=`pwd`/build/doc sysconfdir=`pwd`/build/bin

make -k -i all install install-html install-pdf AM_MAKEINFOHTMLFLAGS="--no-split"
