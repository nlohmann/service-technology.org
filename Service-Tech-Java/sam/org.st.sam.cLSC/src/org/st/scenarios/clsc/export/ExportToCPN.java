...
public class ExportToCPN {
  ...
  private Map<Node, Scenario> originalScenario;

  private Set<Transition> activation_transitions;
  private Set<Place> activation_transition_conditions;
  private Set<Place> min_max_place;
  ...
  // this relationship
  private Map<String, Place> maxPlaces;
   ...
   * 
   * @param hierarchical
   *          put each scenario into a separate subpage of the net
  ...
  public PetriNet translate() {
    ...
    this.originalScenario = new HashMap<Node, Scenario>();
    this.maxPlaces = new HashMap<String, Place>();
    this.min_max_place = new HashSet<Place>();
   ...
   * that activates that chart when its pre-chart occurred. The interface to the
   * rest of the specification (checking occurrence of the pre-chart) and
   * notifying other scenarios about completion of the chart is implemented by
   * fusion places, i.e. places on the page of the scenario that are fused with
   * places on the specification level on the {@link #mainPage}. All places are
   * created by this method, ensuring consistency with the fusion places of
   * other scenarios. The resulting page is added to {@link #net}.
   * 
   * @param s
   * @return
   */
  private Page translateScenario_toPage(Scenario s, Page pg) {
    ...
    for (Event e : main.getEvents()) {
      ...
      e2t.put(e, t);
      originalScenario.put(t, s);

      // and add a place to count occurrences of t
      RefPlace p = toCPN.addFusionPlace(pg, "", "INT", "1`0", e.name
          + "_counter");
      toCPN.addArc(pg, p, t, "j");
      toCPN.addArc(pg, t, p, "j+1");
      originalScenario.put(p, s);

      Chart cHist = s.getLocalHistory(e);

      if (cHist.getEvents().size() > 0) {
        Place pHist = toCPN.addPlace(pg, e.name + "_hist", "TOKENHIST", "1`"
            + translateChartToTokenHistoryString(cHist));
        toCPN.addArc(pg, pHist, t, "hPre");
        toCPN.addArc(pg, t, pHist, "hPre");
        originalScenario.put(pHist, s);
        ...
        for (Event f : cHist.getEvents()) {
          ...
          if (hist_var_count > 0)
            t_activate_incoming_hist += ", ";
          t_activate_incoming_hist += "h" + hist_var_count;
          hist_var_count++;
        }

        // set the guard for activating the scenario
        String t_activate_guard = "[endsWith( joinHistories(["
            + t_activate_incoming_hist + "]),hPre)]";
    ...
    for (Event e : main.getEvents()) {
      ...
      for (Dependency d : e.in) {
        Place p = toCPN.addPlace(pg, d.getSource().name + "_"
            + d.getTarget().name, "TOKENHIST");
        originalScenario.put(p, s);
        ...
        toCPN.addArc(
            ...
            e2t.get(d.getSource()),
            p,
            getTransitionOutGoingArcFunction(d.getSource(),
                getPredecessorEvents(d.getSource(), s)));
      }
      ...
      if (e.isMax()) {
        ...
        if (!maxPlaces.containsKey(e.name)) {
          ...
          originalScenario.put(p, null); // shared place
          min_max_place.add(p);
        }
    ...
    for (Event e : c.getEvents()) {
      ...
      if (e.isMin()) {
        if (dependencies.length() > 0)
          dependencies += ", ";
        dependencies += getDependencyString(getEventString(e, eventNumber),
            getNullEventString());
      }
    }
   ...
   * This is checked by a dedicated activation transition of the chart which is
   * enabled only if there are history-tokens that describe an occurrence of the
   * complete pre-chart. This is the case if each maximal event with label 'A'
   * of the pre-chart produced a token on the global place 'A' with the
   * corresponding history. The activation transition consumes these tokens from
   * the global places and re-produces these tokens for each minimal event of
   * the main-chart on a separate place, i.e. each minimal main-chart event has
   * its own set of pre-places and is concurrent to all other main-chart events.
   * 
   * 
   * @param pg
   * @param s
  ...
  public HashMap<Event, Place[]> translateScenario_activationLogic(Page pg,
    ...
    if (pre == null) {
      ...
      for (Event f : main.getMinEvents()) {
        Place p = toCPN.addPlace(pg, "init", "TOKENHIST",
            "1`{max=[nullEV],deps=[]}");
      ...
      Transition t_activate = toCPN.addTransition(pg,
          "activate_" + s.getName(), "[endsWith(h,hPre)]");
      ...
      originalScenario.put(t_activate, s);
      originalScenario.put(p_preChart, s);
      ...
      for (Event e : pre.getMaxEvents()) {
        ...
        // and produces on the corresponding place for each minimal main-chart
        // event
        for (Event f : main.getMinEvents()) {
  ...
  private static String ROLE_SPEC = "spec";

  public void assignRolesFromScenario() {

    Set<String> allRoles = new HashSet<String>();
    for (Scenario s : originalScenario.values()) {
      if (s == null)
        continue;
      allRoles.add(s.getName());
    }
    ...
    for (Page pg : net.getPage()) {
      for (Object o : pg.getObject()) {
        if (!(o instanceof Node))
          continue;
        Node n = (Node) o;
        roleAssignment.put(n, new LinkedList<String>());
        if (originalScenario.get(n) == null)
          roleAssignment.get(n).addAll(allRoles);
        else
          roleAssignment.get(n).add(originalScenario.get(n).getName());
      }
    }
  }

  public void assignRolesToTransitions(Map<String, String> assignment) {
    for (Page pg : net.getPage()) {
      for (Object o : pg.getObject()) {
  ...
  private void spreadRolesToPlace(PlaceNode p) {
    ...
    //System.out.println(p + " --> " + roleAssignment.get(p));
  }
  
  public void spreadRolesToAllPlaces() {
    for (Page pg : net.getPage()) {
      for (Object o : pg.getObject()) {
  ...
  public Map<Node, List<String>> getRoleAssignment() {
    return roleAssignment;
  }

  public void multiplyActivationLogic() {
    // move scenario activation transitions to a page if all successor
    // transitions
    // also occur on this page
    for (Transition t : activation_transitions) {

      if (roleAssignment.get(t).size() == 1
          && roleAssignment.get(t).contains(ROLE_UNASSIGNED)) {
        ...
        if (successorRoles.size() == 1
          ...
          for (int i = 1; i < _successorRoles.size(); i++) {
            ...
            Transition t2 = toCPN.addTransition(pg, oldTransitionName + "_"
                + successorRole, t.getCondition().getText());
            Place p2 = toCPN.addPlace(pg, activation_place.getName().getText()
                + "_" + successorRole, activation_place.getSort().getText(),
                activation_place.getInitialMarking().getText());
            ...
            for (Arc a : t.getSourceArc()) {
              if (a.getTarget() == activation_place)
                continue;
              toCPN.addArc(pg, t2, a.getTarget(), a.getHlinscription()
                  .getText());
            }
            for (Arc a : t.getTargetArc()) {
              if (a.getSource() == activation_place)
                continue;
              toCPN.addArc(pg, a.getSource(), t2, a.getHlinscription()
                  .getText());
            }
          }
        }
  ...
  public void splitRolesToSubpages(Page oldPage, boolean mergeAllPlaces) {

    Page parentPage = toCPN.addPage(net, ROLE_SPEC);
    ...
    for (String role : roles) {
      ...
      Instance t_page = toCPN
          .createSubPageTransition(subPage, parentPage, role);
    ...
    for (org.cpntools.accesscpn.model.Object o : oldPage.getObject()) {
      if (!(o instanceof Place))
        continue;
      Place p = (Place) o;
      if (!roleAssignment.containsKey(p))
        sharedPlace.add(p);
      if (roleAssignment.get(p).contains(ROLE_UNASSIGNED))
        sharedPlace.add(p);
      if (roleAssignment.get(p).size() > 1)
        sharedPlace.add(p);
    }
    ...
    for (Object o : oldPage.getObject()) {
      ...
      for (Arc a : tgt_arcs) {
        ...
        if (!sharedPlace.contains(p) || pg == parentPage) {
          ...
          RefPlace p_ref = null;
          if (mergeAllPlaces || !min_max_place.contains(p2)) {
            // See if this place is already represented on this page
            // as it references the same place on the topLevel page.
            // If yes: merge them
          ...
          if (p_ref == null)
            p_ref = toCPN.addReferencePlace(pg, p2.getName().getText(), p2
    ...
    for (Arc a : arc_toRemove) {
      ...
      a.getSource().getSourceArc().remove(a);
      a.getTarget().getTargetArc().remove(a);
    }
      ...
      for (Arc a : pg.getArc()) {
        if (a.getSource().getPage() != a.getTarget().getPage()) {
          System.out.println("ERROR: " + a
              + " connects nodes in different pages:");
          System.out.println("   " + a.getSource() + " in "
              + a.getSource().getPage().getName().getText());
          System.out.println("   " + a.getTarget() + " in "
              + a.getTarget().getPage().getName().getText());
        }
      }
    }
   ...
   *         either the predecessors denoted by {@link Event#in}, or the maximal
   *         pre-chart events if 'e' is a minimal main-chart event)
   */
   ...
   * the event that occurred. This function constructs the call to this function
   * by basically collecting the names and history variables of all direct
   * predecessor events of event 'e'.
   * 
   * @param e
   * @param predecessors
   ...
   */
  private String getTransitionOutGoingArcFunction(Event e,
    ...
    fct += "appendEvent((\"" + e.name + "\",j), " + predEvents
        + ", joinHistories( " + predHistories + " ) )";

    return fct;
  }

  /**
   * In the resulting CPN model, incoming arcs of transitions are annotated with
   * history variables. The incoming arcs of each transition need to be
  ...
  private int getNumberOfHistVariables() {
    ...
    for (Scenario s : spec.scenarios) {
      ...
      if (s.preChart != null && numIncoming < s.preChart.getMaxEvents().size())
  ...
  private void addTypeDeclarations() {
    ...
    // histories --- *)
    toCPN.declareMLFunction(net, "val " + getNullEventString() + " = "
        + getEventString("", 0) + ";");

    // colset EVENTLIST = list ID;
    toCPN.declareColorSet_list(net, "EVENTLIST", "ID");
    // colset HISTREL = record event:ID * pred:ID;
    toCPN.declareColorSet_record(net, "HISTREL", new String[] { "event", "ID",
        "pred", "ID" });
    ...
    toCPN.declareColorSet_record(net, "TOKENHIST", new String[] { "max",
        "EVENTLIST", "deps", "EVENTDEP" });
    // colset TOKENHISTLIST = list TOKENHIST;
    toCPN.declareColorSet_list(net, "TOKENHISTLIST", "TOKENHIST");

    // colset EVENTPAIR = product ID * ID;
    toCPN
        .declareColorSet_product(net, "EVENTPAIR", new String[] { "ID", "ID" });
  ...
  /**
   * declare all ML functions for the translation to Petri nets the declarations
   * are read from text files stored in {@value #DIR_ML_DECLARATIONS}
   */
