...
NetState* NetState::createNetStateFromCurrent() {
    ...
    for (int i = 0; i < Net::Card[PL]; i++) {
        ...
        for (int j = 0; j < Net::CardArcs[PL][PRE][i]; j++) ns->Arc[PL][PRE][i][j] = Net::Arc[PL][PRE][i][j];

        ns->Mult[PL][PRE][i] = (mult_t*) malloc(
                                   Net::CardArcs[PL][PRE][i] * SIZEOF_MULT_T);
        for (int j = 0; j < Net::CardArcs[PL][PRE][i]; j++) ns->Mult[PL][PRE][i][j] = Net::Mult[PL][PRE][i][j];


        // post-Zone
        ns->Arc[PL][POST][i] = (index_t*) malloc(
                                   Net::CardArcs[PL][POST][i] * SIZEOF_INDEX_T);
        for (int j = 0; j < Net::CardArcs[PL][POST][i]; j++) ns->Arc[PL][POST][i][j] = Net::Arc[PL][POST][i][j];

        ns->Mult[PL][POST][i] = (mult_t*) malloc(
                                    Net::CardArcs[PL][POST][i] * SIZEOF_MULT_T);
        for (int j = 0; j < Net::CardArcs[PL][POST][i]; j++) ns->Mult[PL][POST][i][j] = Net::Mult[PL][POST][i][j];
    }
    ...
    for (int i = 0; i < Net::Card[TR]; i++) {
        ...
        for (int j = 0; j < Net::CardArcs[TR][PRE][i]; j++) ns->Arc[TR][PRE][i][j] = Net::Arc[TR][PRE][i][j];

        ns->Mult[TR][PRE][i] = (mult_t*) malloc(
                                   Net::CardArcs[TR][PRE][i] * SIZEOF_MULT_T);
        for (int j = 0; j < Net::CardArcs[TR][PRE][i]; j++) ns->Mult[TR][PRE][i][j] = Net::Mult[TR][PRE][i][j];


        // post-Zone
        ns->Arc[TR][POST][i] = (index_t*) malloc(
                                   Net::CardArcs[TR][POST][i] * SIZEOF_INDEX_T);
        for (int j = 0; j < Net::CardArcs[TR][POST][i]; j++) ns->Arc[TR][POST][i][j] = Net::Arc[TR][POST][i][j];

        ns->Mult[TR][POST][i] = (mult_t*) malloc(
                                    Net::CardArcs[TR][POST][i] * SIZEOF_MULT_T);
        for (int j = 0; j < Net::CardArcs[TR][POST][i]; j++) ns->Mult[TR][POST][i][j] = Net::Mult[TR][POST][i][j];
    }
...
NetState* NetState::createNetStateFromCurrent(NetState* ons) {
    ...
    for (int i = 0; i < Net::Card[PL]; i++) {
        ...
        ns->Arc[PL][PRE][i] = (index_t*) malloc(
                                  Net::CardArcs[PL][PRE][i] * SIZEOF_INDEX_T);
        for (int j = 0; j < Net::CardArcs[PL][PRE][i]; j++) ns->Arc[PL][PRE][i][j] = ons->Arc[PL][PRE][i][j];

        ns->Mult[PL][PRE][i] = (mult_t*) malloc(
                                   Net::CardArcs[PL][PRE][i] * SIZEOF_MULT_T);
        for (int j = 0; j < Net::CardArcs[PL][PRE][i]; j++) ns->Mult[PL][PRE][i][j] = ons->Mult[PL][PRE][i][j];


        // post-Zone
        ns->Arc[PL][POST][i] = (index_t*) malloc(
                                   Net::CardArcs[PL][POST][i] * SIZEOF_INDEX_T);
        for (int j = 0; j < Net::CardArcs[PL][POST][i]; j++) ns->Arc[PL][POST][i][j] = ons->Arc[PL][POST][i][j];

        ns->Mult[PL][POST][i] = (mult_t*) malloc(
                                    Net::CardArcs[PL][POST][i] * SIZEOF_MULT_T);
        for (int j = 0; j < Net::CardArcs[PL][POST][i]; j++) ns->Mult[PL][POST][i][j] = ons->Mult[PL][POST][i][j];
    }
    ...
    for (int i = 0; i < Net::Card[TR]; i++) {
        ...
        ns->Arc[TR][PRE][i] = (index_t*) malloc(
                                  Net::CardArcs[TR][PRE][i] * SIZEOF_INDEX_T);
        for (int j = 0; j < Net::CardArcs[TR][PRE][i]; j++) ns->Arc[TR][PRE][i][j] = ons->Arc[TR][PRE][i][j];

        ns->Mult[TR][PRE][i] = (mult_t*) malloc(
                                   Net::CardArcs[TR][PRE][i] * SIZEOF_MULT_T);
        for (int j = 0; j < Net::CardArcs[TR][PRE][i]; j++) ns->Mult[TR][PRE][i][j] = ons->Mult[TR][PRE][i][j];


        // post-Zone
        ns->Arc[TR][POST][i] = (index_t*) malloc(
                                   Net::CardArcs[TR][POST][i] * SIZEOF_INDEX_T);
        for (int j = 0; j < Net::CardArcs[TR][POST][i]; j++) ns->Arc[TR][POST][i][j] = ons->Arc[TR][POST][i][j];

        ns->Mult[TR][POST][i] = (mult_t*) malloc(
                                    Net::CardArcs[TR][POST][i] * SIZEOF_MULT_T);
        for (int j = 0; j < Net::CardArcs[TR][POST][i]; j++) ns->Mult[TR][POST][i][j] = ons->Mult[TR][POST][i][j];
    }
