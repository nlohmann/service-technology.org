 ...
 *
 * \return	the new place which is the result of merging
 */
Place* PetriNet::mergePlaces(Place * & p1, Place * & p2)
{
  if (p1 == p2 && p1 != NULL)
    return p1;		// return this place
 ...
 */
set<Place *> PetriNet::getInternalPlaces() {
...
{
	// TODO: this is a hack and should be solved at the Petri net translation step
    set<Place*> removeNodes;
    set<Node*> noMoreOutput;

// <Dirk.F> removed creation of single final loop (revision 63) and
// <Dirk.F> added a looping place at each output place. #LOOP#
...
//        	Transition* tLoop = newTransition((*place)->nodeFullName() + "_loopFinal");
//        	newArc((*place), tLoop, STANDARD, 1);
//        	newArc(tLoop, (*place), STANDARD, 1);
// <Dirk.F end>
        }
    }

    // make each interface output place a pre-place of the finalizing transition
    // and make it internal (add a new place and remove the old)
    for (set< Place * >::iterator place = P_out.begin(); place != P_out.end(); place ++)
    {
        Place* finalout = newPlace(((*place)->nodeFullName() + "_out"),INTERNAL);

        for (set<Node*>::iterator t = (*place)->preset.begin(); t != (*place)->preset.end();t++) {
            newArc((*t),(finalout), STANDARD, 1);
            noMoreOutput.insert((*t));				// mark pre-transition of interface place
        }

        for (set<Node*>::iterator t = (*place)->postset.begin(); t != (*place)->postset.end();t++) {
            newArc((*t),(finalout), STANDARD, 1);	// mark pre-transition of interface place
        }

// <Dirk.F start> #LOOP#
//        newArc((finalout),finalTransition, STANDARD, 1);
    	Transition* tLoop = newTransition(finalout->nodeFullName() + "_loopFinal");
    	newArc(finalout, tLoop, STANDARD, 1);
    	newArc(tLoop, finalout, STANDARD, 1);
// <Dirk.F end>
        removeNodes.insert((*place));	// mark old interface place for removal
    }
...
// <Dirk.F start> soundness, endNodes and stopnodes
    ...
    // remains true once endNode carries a token
    // (consuming from endNode must not violate the formula)
	for (set<Place *>::iterator it = P.begin(); it != P.end(); it++) {
		Place* p = static_cast<Place*>(*it);

		if (p->nodeFullName().find("endNode") != string::npos )
		{
			// extend endNode-name with .eat and add an arc fromt he corresponding transition
			Transition* t = findTransition(p->nodeFullName() + ".eat");
			newArc(t, p, STANDARD, 1);
		}

		if (p->nodeFullName().find("stopNode") != string::npos )
		{
			// create a looping transition for the stopNode-place
			//Transition* t = newTransition((p->nodeFullName() + ".loop"));
            //newArc(t,p);
            //newArc(p,t);
		}
	}
