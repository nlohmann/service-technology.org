...
/**
 * Algorithm for computing McMillan unfoldings of Petri nets ({@link Petrinet})
 * It initializes and extends a branching process (stored as a {@link DNodeSet})
 * by firing events from a given {@link Petrinet}; the construction
 ...
 * 
 * In addition, behavioral properties of the {@link Petrinet} and can be
 * checked on the fly by setting the <code>options_checkProperty_</code> 
 * fields. 
 * 
 * @author Dirk Fahland
...
public class DNodeBP {
  ...
  private boolean options_checkProperties = false;
	 ...
	 */
  private int options_boundToCheck = 0;
	/**
  ...
  private boolean options_stopIfPropertyFound = true;
	...
	public void configure_PetriNet() {
		...
		options_searchStrat_lexicographic = false;
		options_searchStrat_predecessor = true;
		
		// determine equivalence of cuts by markings
		options_cutOffEquiv_conditionHistory = false;
 ...
 * -------------------------------------------------------------------------- */
  ...
  private LinkedList<DNode> deadConditions;
	 ...
	 */
	public boolean isGloballyBounded() {
		
		if (options_boundToCheck == 0)
			return true;
	...
	public boolean isSafe() {
	  if (options_boundToCheck == 0) return true;
  ...
  /**
   * HV: Not used locally, hence commented out.
   * 
   * @param arr
