...
public class PlaceImpl extends NodeImpl implements Place {
   ...
   */
  public EList<Transition> getPreSet() {
    EList<Transition> preNodes = new BasicEList<Transition>();
    for (Arc a : getIncoming())
      preNodes.add((Transition)a.getSource());
    return preNodes;
  }
   ...
   */
  public EList<Transition> getPostSet() {
    EList<Transition> postNodes = new BasicEList<Transition>();
    for (Arc a : getOutgoing())
      postNodes.add((Transition)a.getTarget());
    return postNodes;
  }
