...
public class Action_BuildPrefix implements IWorkbenchWindowActionDelegate {

	public static final String ID = "hub.top.uma.ui.oclets.BuildPrefix";
    ...
    try {
      ...
      final DNodeBP build = Uma.initBuildPrefix(sys, bound);
      ...
      Job bpBuildJob = new Job("constructing branching process") 
      {
        ...
        protected IStatus run(IProgressMonitor monitor) {
          ...
          monitor.subTask("writing branching process to file");
          
          String targetPathStr = selectedFile_system.getFullPath().removeFileExtension().toString();
          IPath targetPath = new Path(targetPathStr+".bp.dot");
          
          TransitiveDependencies dep = new TransitiveDependencies(build);
          HashSet<DNode> implied = dep.getImpliedConditions_solutionLocal();
          HashMap<DNode, String> coloring = new HashMap<DNode, String>();
          for (DNode b : implied) {
            coloring.put(b, "maroon1");
          }

          ActionHelper.writeFile(targetPath, build.toDot(coloring));
