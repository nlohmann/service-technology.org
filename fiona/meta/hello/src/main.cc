...
void evaluateParameters(int argc, char** argv) {
 ...
 message("arg[ %d ] is %s", i, std::string(argv[i]).data());
...
void terminationHandler() {
    /* [USER] Add code here */
	std::cout << "try to start here...\n";

	message("invocation : %s ", invocation.data() );
//	message("wendy_out : %s ", wendy_out.data() );
...
int main(int argc, char** argv) {
    ...
    // og output hard wire
//    outputParam += " -o ";
//    std::string outputOG = std::string( args_info.inputs );
//    outputParam += outputOG;
    outputParam += "--o=test.hello.og";
 ...
 message("3. set output parameter for Wendy. \n outputParam : %s ", outputParam.data());

 message("4. call Wendy and pipe the normalized net to Wendy.");
...

    {
        ...
        message("ss : .owfn "); //, ss.str().c_str());
        message("command_line : %s ", command_line.c_str());
    ...
    message("5. parse Wendy's output and write out the result to the shell.");
    /*--------------------------------------------------------------.
    ...
    if (not args_info.output_given) {
        ...
        message("opening file : %s ", fileName.c_str());
