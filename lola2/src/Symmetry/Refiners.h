...
protected:
    ...
    virtual bool refineNeighbours(index_t)
    {
        return false;
    };
    virtual bool refineMultiplicity(index_t)
    {
        return false;
    };
    virtual split_cell_t getInfo(index_t i, index_t cell, index_t *repr)
    {
        split_cell_t info = {0, 0};
        return info;
    };
    virtual index_t getCardinality(index_t)
    {
        return 0;
    };
public:
...
public:
    ...
    cmp_t compare(index_t v, index_t w)
    {
        ...
        normalize(&v);
        normalize(&w);
        if (Transition::Fairness[v] < Transition::Fairness[w])
        {
            return CMP_LT;
        }
        if (Transition::Fairness[v] > Transition::Fairness[w])
...
class NeighbourRefinement : public CountingRefinement, public DirectionalRefiner,
    public RepresentativeRefiner, public NetRefiner
{
protected:
    index_t vertex;
    node_t node;
public:
    NeighbourRefinement(Partition *p, Partition *r, index_t v, direction_t dir) : CountingRefinement(p),
        DirectionalRefiner(dir),  RepresentativeRefiner(r), vertex(v), node(normalize(&vertex)) {};
    index_t value(index_t v)
    {
...
class SCCNeighbourRefinement : public CountingRefinement, public DirectionalRefiner,
    public SCCRefiner, public RepresentativeRefiner
{
protected:
    index_t vertex;
public:
    SCCNeighbourRefinement(Partition *p, SCC *s, Partition *r, index_t v,
                           direction_t dir) : CountingRefinement(p), DirectionalRefiner(dir), SCCRefiner(s),
        RepresentativeRefiner(r), vertex(v) {};
    index_t value(index_t v)
    {
        return repr->representative[scc->arcs[direction][vertex][v]];
    };
};
...
protected:
    ...
    node_t node;
public:
    MultRefinement(Partition *p, index_t v, direction_t dir) : ComparisonRefinement(p),
        DirectionalRefiner(dir), vertex(v), node(normalize(&vertex)) {};
    cmp_t compare(index_t v, index_t w)
    {
        if (Net::Mult[node][direction][vertex][v] < Net::Mult[node][direction][vertex][w])
        {
            return CMP_LT;
        }
        if (Net::Mult[node][direction][vertex][v] > Net::Mult[node][direction][vertex][w])
...
protected:
    index_t vertex;
public:
    SCCMultRefinement(Partition *p, SCC *s, index_t v, direction_t dir) : ComparisonRefinement(p),
        DirectionalRefiner(dir), SCCRefiner(s), vertex(v) {};
    cmp_t compare(index_t v, index_t w)
    {
        if (scc->mult[direction][vertex][v] < scc->mult[direction][vertex][w])
        {
            return CMP_LT;
        }
        if (scc->mult[direction][vertex][v] > scc->mult[direction][vertex][w])
...
class SCCPartitionRefinement : public CountingRefinement, public RepresentativeRefiner,
    public NetRefiner, public SCCRefiner
{
public:
    SCCPartitionRefinement(Partition *p, SCC *s, Partition *sp) : CountingRefinement(p),
        RepresentativeRefiner(sp), SCCRefiner(s) {};
    index_t value(index_t v)
    {
        return repr->representative[scc->scc[normalize(&v)][v]];
    };
};
...
public:
    ...
    index_t value(index_t v)
    {
        return 1 + scc->ranges[v].to - scc->ranges[v].from;
    };
};
...
public:
    DagRefinement(SCC *s, direction_t dir) : DirectionalRefiner(dir), SCCRefiner(s), head(0), next(0)
    {
        ...
        for (index_t i = 0; i < scc->sccs; i++) if (scc->card[complement][i] == 0)
            {
                queue[next++] = i;
            }
        next %= scc->sccs;
...
class SCCNeighbourhoodRefinement : public NetRefiner, public DirectionalRefiner, public SCCRefiner,
    public NeighbourhoodRefiner
{
protected:
...
public:
    SCCNeighbourhoodRefinement(Partition *sp, SCC *s, direction_t dir) : DirectionalRefiner(dir),
        SCCRefiner(s), NeighbourhoodRefiner(sp) {};
};

class NeighbourhoodRefinement : public DirectionalRefiner, public NeighbourhoodRefiner,
    public NetRefiner
{
protected:
    index_t getCardinality(index_t);
    ...
    bool refineMultiplicity(index_t);
public:
    NeighbourhoodRefinement(Partition *p, direction_t dir) : DirectionalRefiner(dir),
        NeighbourhoodRefiner(p) {};
};
...
{
    T *b1 = new T[length];
    memcpy(b1, a1, sizeof(T) * length);
    std::sort(b1, &b1[length]);
    T *b2 = new T[length];
    memcpy(b2, a2, sizeof(T) * length);
    std::sort(b2, &b2[length]);
