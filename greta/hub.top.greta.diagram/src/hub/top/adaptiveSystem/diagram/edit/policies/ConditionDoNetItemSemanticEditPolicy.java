 ...
 * @generated
 */
public class ConditionDoNetItemSemanticEditPolicy
		extends
	...
	/**
	 * @author Manja Wolf
	 * check during creation of arcToCondition that the arc is in the same parentNet (Oclet or AdaptiveProcess) 
	 * 
	 */
	public boolean equalParentNet(CreateRelationshipRequest req) {
		//System.out.println("ConditionDoNetItemSemanticEditPolicy.equalParentNet(req) START.");
		if (req.getSource() != null && req.getTarget() != null) {
			...
			if (req.getSource().eContainer() instanceof DoNet
				...
				MessageDialog.openError(shell, "AdaptiveSystem - create arc",
						"An arc from DoNet to PreNet is not allowed.");
				return false;
			}
	...
	public boolean equalParentNet(ReorientRelationshipRequest req) {
		//System.out.println("ConditionDoNetItemSemanticEditPolicy.equalParentNet(ReorientReq) START.");
		if (req.getNewRelationshipEnd() != null
			...
			if(!req.getOldRelationshipEnd().eContainer().equals(req.getNewRelationshipEnd().eContainer())) {
				System.out.println("An arc from oclet to adaptive process is not allowed.");
				Shell shell = new Shell();
				MessageDialog
					.openError(shell, "AdaptiveSystem - reorient arc",
						"An arc from a node in an oclet to one in adaptive process is not allowed.");
				return false;
			}
			 */
