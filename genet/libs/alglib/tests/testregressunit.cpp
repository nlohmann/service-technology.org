...
bool testlinregression(bool silent)
{
    ...
    for(n = 2; n <= maxn; n++)
    {
        ...
        for(pass = 1; pass <= passcount; pass++)
        {
            ...
            generatetask(ea, eb, -5*ap::randomreal(), +5*ap::randomreal(), ap::randomreal()>0.5, double(1), double(1), n, xy, s);
            lrlines(xy, s, n, info, a, b, vara, varb, covab, corrab, p);
            ...
            {
                slerrors = slerrors||fabs(a-a2)>100*ap::machineepsilon||fabs(b-b2)>100*ap::machineepsilon;
            }
            ...
            if( n>2 )
            {
                for(i = 0; i <= qcnt-1; i++)
                {
                    if( fabs(qtbl(i)-qvals(i))/qsigma(i)>sigmathreshold )
                    {
                        slerrors = true;
                    }
                }
    ...
    for(n = 3; n <= maxn; n++)
    {
        ...
        gropterrors = gropterrors||fabs(a-tmpweights(1))>1000000*ap::machineepsilon;
        gropterrors = gropterrors||fabs(b-tmpweights(0))>100000*ap::machineepsilon;
        grcoverrors = grcoverrors||fabs(vara-ar.c(1,1))>100000*ap::machineepsilon;
        grcoverrors = grcoverrors||fabs(varb-ar.c(0,0))>100000*ap::machineepsilon;
        grcoverrors = grcoverrors||fabs(covab-ar.c(1,0))>100000*ap::machineepsilon;
        grcoverrors = grcoverrors||fabs(covab-ar.c(0,1))>100000*ap::machineepsilon;
        ...
        gropterrors = gropterrors||fabs(a-tmpweights(1))>100000*ap::machineepsilon;
        gropterrors = gropterrors||fabs(b-tmpweights(0))>100000*ap::machineepsilon;
    }
    ...
    {
        grcoverrors = grcoverrors||fabs(log(ar.c(0,0)/varb))>log(1.2);
        grcoverrors = grcoverrors||fabs(log(ar.c(1,1)/vara))>log(1.2);
        grcoverrors = grcoverrors||fabs(log(ar.c(0,1)/covab))>log(1.2);
        grcoverrors = grcoverrors||fabs(log(ar.c(1,0)/covab))>log(1.2);
    }
        ...
        for(tasktype = 0; tasktype <= 3; tasktype++)
        {
            ...
            for(m = m1; m <= m2; m++)
            {
                ...
                for(n = n1; n <= n2; n++)
                {
                    ...
                    grothererrors = grothererrors||fabs(v-lrprocess(wt, x))/fabs(v)>1000*ap::machineepsilon;
                    ...
                    v = lrprocess(wt, x);
                    grothererrors = grothererrors||fabs(v-lrprocess(wt2, x))/fabs(v)>1000*ap::machineepsilon;
    ...
    for(pass = 1; pass <= 50; pass++)
    {
        ...
        if( info==1 )
        {
            ...
            grothererrors = grothererrors||fabs(v-noiselevel*sqrt(double(2)/double(3)))>1000*ap::machineepsilon;
            v = lravgerror(wt, xy, 3*n);
            grothererrors = grothererrors||fabs(v-noiselevel*(double(2)/double(3)))>1000*ap::machineepsilon;
            v = lravgrelerror(wt, xy, 3*n);
            grothererrors = grothererrors||fabs(v-(fabs(noiselevel/(tasklevel-noiselevel))+fabs(noiselevel/(tasklevel+noiselevel)))/3)>1000*ap::machineepsilon;
        }
        else
        {
            grothererrors = true;
        ...
        if( info==1 )
        {
            lrunpack(wt, tmpweights, tmpi);
            v = lravgrelerror(wt, xy, 3*n);
            grothererrors = grothererrors||fabs(v-1)>1000*ap::machineepsilon;
        }
        else
        {
            grothererrors = true;
...
bool testregressunit_test_silent()
{
    ...
    return result;
#else
    return _i_testregressunit_test_silent();
#endif
}
...
bool testregressunit_test()
{
    ...
    return result;
#else
    return _i_testregressunit_test();
#endif
}
