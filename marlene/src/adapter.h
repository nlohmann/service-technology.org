...
class Adapter {
  ...
  public:
    ...
    Adapter( std::vector< PetriNet_ptr > & nets, RuleSet & rs,
        ControllerType contType = SYNCHRONOUS, unsigned int messagebound = 1,
  ...
  private:
    ...
    //! map of transitions and their communication label toward the controller
    //! needed for adaptability check
    std::map< std::string, std::string > controllerInterfaceSync;

    //! map of transitions and their communication label toward the wanted net
    //! needed for adaptability check
    std::map< std::string, std::string > netInterfaceIn;
    std::map< std::string, std::string > netInterfaceOut;
    std::map< std::string, std::string > netInterfaceSync;
     ...
     */
    static void
    createComplementaryPlaces( pnapi::PetriNet & net );
...
class RuleSet {
  ...
  private:
     ...
     */
    unsigned int
    getIdForMessage( std::string message );
