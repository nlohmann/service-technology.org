...
AC_PATH_PROG(LOLA, [lola-statepredicate1], [])
if test "${LOLA}" = ""; then
  ...
  make lola-statepredicate1 > /dev/null 2> /dev/null
  make install > /dev/null 2> /dev/null
  cd ..

  cp "$LOLADIR"local/bin/lola-statepredicate1* .
  rm -fr $LOLADIR lola.tar.gz

  AC_PATH_PROG(LOLA, [lola-statepredicate1], [])
else
AC_PATH_PROGS_FEATURE_CHECK([LOLAVERSION], [lola-statepredicate1], [[ac_cv_path_LOLAVERSION=`$ac_path_LOLAVERSION --version | ${SED} -e 's/LoLA //'`]])
