...
typedef struct{
	std::map<unsigned int,unsigned int> msleft;
	std::multiset<unsigned int, unsigned int> msright;
} adrule;
set<const adrule *> sea;
...
int main(int argc, char** argv) {
	...
	lprec *lpfing=NULL;//adapter fingerprint
	//std::set<lprec *> retlpset;//set of all final markings of the composition I do not need this any more
			...
			if(i==0){			
				...
				if (!syntb) {
					...
					for (set<string>::iterator ssit=inputplaces1.begin(); ssit!=inputplaces1.end(); ++ssit) {
						...
						Place *p=net1.findPlace(*ssit); //cout<<"was interface"<<p->wasInterface()<<" "<<p->getName()<<endl;
						//cout<<endl<<"found"<<*itwas.find(*ssit)<<endl; && itwas.find(*ssit)!= itwas.end()
						if(p!=NULL  && itwas.find(*ssit)!=itwas.end()){ //create complementary
							//create place
							Place * place = net1.findPlace(p->getName()+"_compl");
							...
							if ( place == NULL )
							{
								...
								{//create a post rtansition for the complementary
										...
										net1.createArc( *net1.findPlace(p->getName()+"_compl"),(*f)->getTransition(), (*f)->getWeight());
								...
								for (set<Arc *>::iterator f = postset.begin(); f != postset.end(); ++f)
								{
										...
										//if (net1.findPlace(place->getName()+"_compl")!=NULL) {
											...
											net1.createArc( (*f)->getTransition(), *pc,(*f)->getWeight());
										//}
								}
					...
					for (set<string>::iterator ssit=outputplaces1.begin(); ssit!=outputplaces1.end(); ++ssit) {
						Place *p=net1.findPlace(*ssit);//cout<<"was interface"<<p->wasInterface()<<" "<<p->getName()<<endl;
													//create place
							Place * place = net1.findPlace(p->getName()+"_compl");//cout<<*ssit<<endl;
					...
					string name1=args_info.inputs[0]; string name2=args_info.inputs[1];
					Output output(folder+name1+name2, "open net");
					output.stream() << pnapi::io::owfn << net1; exit(0);
					//net1.compose(net2, s1, s2);
				}
				
				/*if(args_info.composable_flag){
					//maybe
					set<Label *> l1a=net1.getInterface().getAsynchronousLabels();
					set<Label *> l1s=net1.getInterface().getSynchronousLabels();
					set<Label *> l2a=net2.getInterface().getAsynchronousLabels();
					set<Label *> l2s=net2.getInterface().getSynchronousLabels();
				}*/
				...
				net1.compose(net2, s1, s2);//cout<< s1<<"   "<<s2<<endl;
				if ( args_info.adapterrules_given )
				{//adapter
    ...
    if ( args_info.adapterrules_given )
    {
		...
		//	delete netps[index];
		//}
		//build an adapter fingerprint in case we have fingerprints
		
		//create adapter fingerprint 
		
		
		std::list< RuleSet::AdapterRule * > rules =rs.getRules();
		// iterate them
		std::list< RuleSet::AdapterRule * >::iterator ruleIter = rules.begin();
		std::set<std::string> base;//global set of variables
		while ( ruleIter != rules.end() )
		{
			const RuleSet::AdapterRule & rule = **ruleIter;
			
			//nocolumns			unsigned int nbcol=0;
			...
			{
				// get the interface place name
				std::string intp = rs.getMessageForId(*messageIter);
				//cout << intp<<" ";
				if (base.find(intp)==base.end()) base.insert(intp);
				// is the place  already in the adapter fingerprint
				//get place
				//if ( place == NULL ){//create a new column for it otherwise 
					
				//}
				//else{ // add to multiplicity
				//}
				messageIter++;
			}
			...
			while ( messageIter != messageList.end() )
			{
				// get interface place name
				std::string intp = rs.getMessageForId(*messageIter) ;
				//cout << intp<<" ";
				if (base.find(intp)==base.end()) base.insert(intp);
				// isthe place  already in the adapter fingerprint
			...
			while ( messageIter != messageList.end() )
			{
				...
				std::string labelName = rs.getMessageForId(*messageIter);
				syncLabel.insert(labelName);
				if (base.find(labelName)!=base.end()) base.insert(labelName);
				messageIter++;
			}
		...
		//cout<<"column number "<<base.size()<< " "<<endl;
		
		lpfing = make_lp(0, base.size());
		
		for(int i=1;i<=get_Ncolumns(lpfing);i++){
			set_int(lpfing,i,TRUE);
		}
		if(lpfing == NULL) {
			std::cerr<< "Unable to create new LP model"<<std::endl;
			delete adapter;
		...
		int k=1;
		std::string prevs=*base.begin();
		//cout<<"size "<<base.size()<< " "<<endl;
		for(std::set<std::string>::iterator cit=base.begin();cit!=base.end();++cit){
			char * cstr= new char [(*cit).size()+1];
			strcpy(cstr,(*cit).c_str());
			set_col_name(lpfing, k, cstr);k++;
			if (cit==base.begin()) {
				continue;//when we have just one 
			}
			else {//add the equality constraint 
				
				//int colno[2];
				int * colno=new int[1+2]();
				REAL * sparserow= new REAL[1+2]();
				colno[0]=get_nameindex(lpfing, const_cast<char *>((*cit).c_str()), FALSE);//get nameindex for current
				//cout<<"column "<<*cit<<colno[0]<< " "<<endl;
				colno[1]=get_nameindex(lpfing, const_cast<char *>(prevs.c_str()), FALSE);//get get nameindex for previous
				//cout<<"column "<<prevs<<colno[1]<< " "<<endl;
				//REAL sparserow[2];//we have 2 values always
				//we suppose that the events occur just once
				sparserow[0]=1.0;//this is the coefficient of the second 
				sparserow[1]=-(int)1.0;//this is the coefficient of the first
				//get_nameindex(lp,const_cast<char *>(t.getName().c_str()),FALSE)
				if(!add_constraintex(lpfing,2,sparserow, colno,EQ, 0.0))//lp, 2, sparserow, colno, GE, 3.0
					cout<<"gata"<<endl;
				delete[] colno;delete[] sparserow;
			}
			prevs=*cit;
			
		}
		//for each two add an equality
	...
	if(args_info.messageProfiles_given>0){
		...
		bool syncb=false;
		std::set<std::string> inputp,outputp, syncp; // variables necessary for the composition for the previous
		...
		for (set<std::string>::iterator sit=hh.begin(); sit!=hh.end(); ++sit) { //for each net
			...
			for(std::multimap<std::string,int>::iterator it=mpinvocation.begin();it!=mpinvocation.end();++it){ //for each mp
				...
				//check whether we can make the composition by making the difference with the previous sset of places
				//if()inputplaces1.find(ps)!=inputplaces1.ensitd()
				if (sit!=hh.begin()) {
					syncb=false;
					cout << inputPlaces.size()<< outputPlaces.size()<<synchrT.size()<<"places"<<endl;
					cout << inputplaces1.size()<< outputplaces1.size()<<"places1"<<endl;
					for (std::set<std::string>::iterator ssit=outputplaces1.begin(); ssit!=outputplaces1.end(); ++ssit) {
					// we can make composition for several nets !
						cout<<*ssit<<" ";
						if(inputPlaces.find(*ssit)!=inputPlaces.end()) syncb=true;
							//inputplaces1.insert(*ssit);
					}
					for (std::set<std::string>::iterator ssit=inputplaces1.begin(); ssit!=inputplaces1.end(); ++ssit) {
						// we can make composition for several nets !
						cout<<*ssit<<" ";
						if(outputPlaces.find(*ssit)!=outputPlaces.end()) syncb=true;
						//inputplaces1.insert(*ssit);
					}		
					if (!syncb) {
						std::cout<<endl<<"not syntactically compatible"<<endl; exit(0);
				...
				if (no == 1) { //get interface it==mpinvocation.begin()//first time 
					cout <<"here1"<< inputPlaces.size()<< outputPlaces.size()<<synchrT.size()<<endl;
					cout << inputp.size()<< outputp.size()<<synct.size()<<endl;
					cout << resultinp.size()<< resultout.size()<<resultsyn.size()<<endl;
...
/**/
					...
					for (std::set<std::string>::iterator ssit=outputPlaces.begin(); ssit!=outputPlaces.end(); ++ssit) {
						outputplaces1.insert(*ssit);
					}
					cout<<inputplaces1.size()<<outputplaces1.size()<<"adding"<<endl;
				...
				for(unsigned int i=0;i<mps.size();++i){
					...
					for(unsigned int j=0;j<lpmps.size();++j){// add lpr too
						...
						for (int r=0; r<get_Nrows(lpmps.at(j)); ++r) {
							...
							//xsif( grex==-1) cout<<"gata";
							//add_constraint(lpr,rowp,get_constr_type(lpmps.at(j),r+1),get_rh(lpmps.at(j),r+1));
							if(!add_constraintex(lpr,count,rowe,colno,get_constr_type(lpmps.at(j),r+1),get_rh(lpmps.at(j),r+1))){
		...
		//cout << inputPlaces.size()<< outputPlaces.size()<<synchrT.size()<<endl;
		//cout << inputp.size()<< outputp.size()<<endl;
		//cout << resultinp.size()<< resultout.size()<<resultsyn.size()<<"gata"<<endl;
		//mps+lpmps
		// union of variables
		status("finished adding all message profile files");

		
		//pt diferenta adauga variabile if (args_info.composable_flag) {;}
		//insert 
		//solve
		if (args_info.inputs_num==0) {
			status("add mappings for adapters");
			if (args_info.verbose_flag) {print_lp(lpfing);}
			for(unsigned int ifm=0;ifm<lpmps.size();++ifm){
				if(args_info.adapterrules_given){//add adapter fingerprint
					// first add variables 
					for (int i=0; i<get_Ncolumns(lpfing); ++i) { //add variables from previous
						//std::string pr(get_col_name(lpmps.at(ifm),i+1));
						status("%s",get_col_name(lpfing,i+1));
						if(get_nameindex(lpmps.at(ifm), get_col_name(lpfing,i+1), FALSE)==-1){
							//add new zero column
							add_columnex(lpmps.at(ifm),0,NULL,NULL);
							set_col_name(lpmps.at(ifm), get_Ncolumns(lpmps.at(ifm)),get_col_name(lpfing,i+1));
						}
					/*	if (result.find(pr)==result.end()) {//add columns from old composition to the new interface
							result.insert(pr);
							//REAL column[1+get_Nrows(lpmps.at(0))];
					...
					set_add_rowmode(lpmps.at(ifm), TRUE);
					for(int i=0;i<get_Nrows(lpfing);++i){
						REAL * rowp=new REAL[1+get_Ncolumns(lpfing)](); 
						//REAL * rowwpp=new REAL[hh]();//int *colp;
						char grex=get_row(lpfing,i+1,rowp);//status(" adding fingerprint");
						//for(int j=0;j<hh-1;j++)rowwpp[j]=0;
				        //for(int j=0;j<get_Ncolumns(lpfing);j++){rowwpp[j+hh-get_Ncolumns(lpfing)]=rowp[j+1];}
						//cout<<endl;for(int j=0;j<hh;j++){cout<<rowwpp[j]<<" ";}
						if( grex==-1) cout<<"gata";
						add_constraint(lpmps.at(ifm),rowp,get_constr_type(lpfing,i+1),get_rh(lpfing,i+1));
					...
					set_add_rowmode(lpmps.at(ifm), FALSE);
				}
				...
				if (args_info.verbose_flag) {print_lp(lpmps.at(ifm));}
	...
	if(args_info.inputs_num>0){//args_info.adapterrules_given){//args_info.messageProfiles_given>0
		...
		if(nofinal) nofinal=nofinal*fmset.size();
	    else {
			nofinal=fmset.size();
		}
		...
		for(std::set<Place *>::const_iterator p = net1.getPlaces().begin();p!=net1.getPlaces().end();p++){
			...
			if(inmk.find((*p)->getName())!=inmk.end()){
		//for (std::map<std::string const, unsigned int> mit=inmk.begin(); mit!=mm.end(); ++mit) {
			...
			delete[] colno;delete[] rowe;delete[] roww;
		...
		//for(set<lprec *>::iterator citr = retlpset.begin();citr!=retlpset.end();citr++){
		unsigned int origlpr=get_Nrows(lp);unsigned int origlpcol=get_Ncolumns(lp);
		for(set<map<std::string const ,unsigned int> >::iterator sfm=fmset.begin();sfm!=fmset.end();++sfm){
			int ii=0; 
			if(sfm!=fmset.begin()) resize_lp(lp, origlpr,origlpcol);
//here starts the building of the state equation (should be done only once)
/*			lprec *lp;//lp = make_lp(0, nTransitions);
...
//here comes the right hand side(should remain in the final marking iteration)
//add resize_lp(lp,rows, columns) rows should be the same and columns
			    ///to set the rhs, we need to iterate all places marked by initial and final marking and call set_rhs  
				
			std::map<std::string const, unsigned int> mmm=(*sfm); ///mit=sfm->find(ssss);
			for (std::map<std::string const, unsigned int>::iterator mit=mmm.begin(); mit!=mmm.end(); ++mit) {
				const std::string hs=(*mit).first;
					char *cstr= new char [hs.size()+1];
					strcpy(cstr,hs.c_str());//needed by get row by index
					int irow = get_nameindex(lp, cstr, TRUE);
					if(inmk.find(mit->first)!=inmk.end())
						set_rh(lp, irow, 0.0+mmm[cstr]+get_rh(lp,irow));
					else {
						set_rh(lp, irow, 0.0+mmm[cstr]);
					}
...
//aici				if(!add_constraintex(lp,k,rowe,colno, EQ, rhs)) cout<<"nu merge"<<endl;
				//if(!add_constraint(lp, roww,EQ, rhs))
				//	cout<<"gata"<<endl;
//si aici				set_row_name(lp,get_Nrows(lp), cstr);
				//add the constrain's right side
				//get the row with the palce row name
				//cout<<endl<<get_nameindex(*citr,cstr,TRUE)<<" "<<get_rh(*citr, get_nameindex(*citr,cstr,TRUE))<<endl;
				// //			set_row_name(lp, i, cstr);			
			...
			if(args_info.messageProfiles_given>0){
				...
				//for each final marking of the fingerprint add constraints and variables
			...
			if(args_info.enforceEvents_given>0){
				...
				if(args_info.verbose_flag){print_lp(lp);}
				...
				else{if (args_info.verbose_flag) {std::cout<<endl<<"So far no solution  ..."<<endl;}}
			}
		}
		
	}
	std::cout<<endl<<"Size of the set of final markings "<<nofinal<<endl;
	if(res!=0){
