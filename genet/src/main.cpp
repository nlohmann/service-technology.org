...
int genet_init(int argc,char *argv[]) {
    ...
    while ((optind < argc) /*&& (argv[optind][0]!='<')*/) {
        ...
        if (sw=="-k") {
            ++optind;
            KMAX = atoi(argv[optind]);
            cout << "#  -k " << KMAX <<endl;
        }
        else if (sw=="-exp") {
            ++optind;
            explore_regions = atoi(argv[optind]);
            cout << "#  -exp " << explore_regions <<endl;
        }
        else if (sw=="-min") {
            ++optind;
            minimal_k = atoi(argv[optind]);
            cout << "#  -min " << minimal_k <<endl;
        }
        else if (sw=="-pm") {
            on_demand = false;
            cout << "#  -pm " <<endl;
            ...
            ++optind;
            MAX_RCOV = atoi(argv[optind]);
            cout << "#  -cov " << MAX_RCOV <<endl;
        }
        else if (sw=="-enc") {
            ++optind;
            enc_method = atoi(argv[optind]);
            cout << "#  -enc " << enc_method <<endl;
        }
        else if (sw=="-hide") {
            ++optind;
            hide_sigs = argv[optind];
            hiding = true;
            cout << "#  -hide " << hide_sigs <<endl;
        }
        else if (sw=="-prj") {
            ++optind;
            project_string = argv[optind];
            project = true;
            cout << "#  -prj " << project_string <<endl;
        }
        else if (sw=="-ec") {
            ++optind;
            ec_string = argv[optind];
            force_ec_sigs = true;
            cout << "#  -ec " << ec_string <<endl;
        }
        else if (sw=="-h") {
            ++optind;
            genet_usage();
