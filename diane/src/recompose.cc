...
pnapi::PetriNet addPattern(pnapi::PetriNet nnn) {
    ...
    diff.createArc(*pc, *tf, 1);
    diff.createArc(*tf, *pa, 1);
    diff.createArc(*pb, *tf, 1);
    diff.createArc(*tf, *pb, 1);
    diff.createArc(*pc, *tg, 1);
    diff.createArc(*tg, *pb, 1);
    diff.createArc(*tg, *pe, 1);
    diff.createArc(*pd, *tg, 1);
    ...
    for (set<Label*>::iterator pp = inputpp.begin(); pp != inputpp.end(); ++pp) {
        ...
        for (set<Transition*>::iterator t = stt->begin(); t != stt->end(); ++t) {
            if (diff.findArc(*pa, **t) == NULL) {
                diff.createArc(*pa, **t, 1);
            }
            if (diff.findArc(**t, *pc) == NULL) {
                diff.createArc(**t, *pc, 1);
            }
        }
    }
            ...
            if ((!(*p)->getPresetArcs().empty()) && (!(*p)->getPostsetArcs().empty())) {
                ...
                if (cmplfordiff.findPlace((*p)->getName() + "$compl") != NULL) {
                    ...
                    for (set<pnapi::Arc*>::iterator f = preset.begin(); f != preset.end(); ++f) {
                        Transition* t = &(*f)->getTransition();
                        //nets[j]->createArc(**p ,*t, (*f)->getWeight());
                        if (cmplfordiff.findArc(*t, *nplace) == NULL) {
                            cmplfordiff.createArc(*t, *nplace, (*f)->getWeight());
                        }
    ...
    for (std::set<pnapi::Label*>::iterator p = async.begin(); p != async.end(); ++p) {
        if ((*p)->getType() == Label::INPUT)
            if (diffc.findPlace((*p)->getName())) {
                ...
                diff.deletePlace(*diff.findPlace((*p)->getName()));
                if (diff.findPlace((*p)->getName() + "$compl")) {
...
bool areComposable(pnapi::PetriNet net1, pnapi::PetriNet net2) {
    ...
    for (std::set<pnapi::Label*>::iterator cit = synclabels.begin(); cit != synclabels.end(); ++cit) {
        if (net2.getInterface().findLabel((*cit)->getName()) != NULL) {
            syntb = true;
            break;//not sync
        }
    }
...
bool noInterfaceLabel(set<string> sset, string place) {
    ...
    for (set<string>::iterator sit = sset.begin(); sit != sset.end(); ++sit) {
        if (strstr(place.c_str(), sit->c_str()) == NULL) {
            return true;
        }
    }
    return false;
...
vector<string> setPlaceOrder(PetriNet net) {
    ...
    for (std::set<pnapi::Place*>::iterator p = net.getPlaces().begin(); p != net.getPlaces().end(); ++p) {
        if (strstr((*p)->getName().c_str(), "Control") != NULL) {
            sop.push_back((*p)->getName());
        }
    }
    for (std::set<pnapi::Place*>::iterator p = net.getPlaces().begin(); p != net.getPlaces().end(); ++p) {
        if (strstr((*p)->getName().c_str(), "Data") != NULL) {
            sop.push_back((*p)->getName());
        }
    }
    for (std::set<pnapi::Place*>::iterator p = net.getPlaces().begin(); p != net.getPlaces().end(); ++p) {
        if (strstr((*p)->getName().c_str(), "Control") == NULL && strstr((*p)->getName().c_str(), "Data") == NULL) {
            rest.push_back((*p)->getName());
        }
    }
...
PetriNet makeComponentNet(Component c) {
    ...
    for (std::set<string>::iterator ssit = c.inputl.begin(); ssit != c.inputl.end(); ++ssit) {
        ...
        if (netLabel == NULL) {
            netLabel = &cn.getInterface().addInputLabel(*ssit);
        }
    ...
    while (!sdq.empty()) {
        ...
        for (set<pnapi::Arc*>::iterator f = postset.begin(); f != postset.end(); ++f) {
            ...
            if ((cn.findPlace(torig.getName()) == NULL) && (asyncl1.find(torig.getName()) == asyncl1.end())) { // and not interface
                ...
                adjacentn = setUnion(adjacentn, r->getPreset());
                adjacentn = setUnion(adjacentn, r->getPostset());
                pi = &cn.createPlace(r->getName(), r->getTokenCount()); // create place and arc
            }
            if (asyncl1.find(torig.getName()) == asyncl1.end()) {
                cn.createArc(*ytc, *pi, (*f)->getWeight());
            }
        }
...
vector<Component> recompose(vector<PetriNet*> nets, PetriNet net) {
    ...
    std::vector<bool> in(nc, false), com(nc, false);
    in.flip();
        ...
        if (parts.at(i).getPlaces().size() + 5 <= 2 * maxi / 3) {
            ...
            for (unsigned int j = i + 1; j < parts.size(); ++j) {
                ...
                if (areComposable(parts.at(i), parts.at(j)) && (parts.at(i).getPlaces().size() + parts.at(j).getPlaces().size() + 10 < maxi * 2 / 3)) {
                    ...
                    cout << "not syntactically compaible" << endl;
                }
        ...
        for (int i = 0; i < (int) parts.size(); i++) {
            ...
            for (it = small.begin() ; it != small.end(); ++it) {
                ...
                if (!(*it).first[i]) {// i+i is forbidden by default
                    ...
                    vector<bool> vi(nc, false);
                    vi[i] = true;
                    std::pair<vector<bool>, PetriNet> pi;
                    pi.first = vi;
                    pi.second = parts.at(i);
