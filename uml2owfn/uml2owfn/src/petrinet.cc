/*****************************************************************************\
  ...
  Copyright (C) 2008  Martin Znamirowski <znamirow@informatik.hu-berlin.de>,
                      Dirk Fahland <dirk.fahland@service-technolog.org>,
  Copyright (C) 2007  Martin Znamirowski,
                      Dirk Fahland,
                      Niels Lohmann <niels.lohmann@service-technolog.org>
  Copyright (C) 2006  Christian Gierds,
                      Niels Lohmann, and
                      Martin Znamirowski
...
/*!
 * \brief   utility function to locate a specific substring in the role history
 *          of a Petri net node
 *
 * \param substring to be found
 *
 * \return  true iff the history contains an entry with the given substring
 */
bool Node::historyContainsSubString(string substring) const {
  for (list<string>::const_iterator hist_it = history.begin(); hist_it != history.end(); hist_it++) {
    if ((*hist_it).find(substring) != string::npos)
      return true;
  }
  return false;
}
...
// returns true if the place has at least one token
bool Place::marked()
{
  return (tokens >= 1);
}
...
//  cerr << "new arc: " << my_source->id << "(" << my_source->nodeFullName() << ") -> " << my_target->id << "(" << my_target->nodeFullName() << ")" << endl;
...
PetriNet & PetriNet::operator=(const PetriNet & net)
{
  ...
  roleMap.clear();
/* DECOMPOSITION
  for (set< Place * >::iterator place = net.generatedInitials.begin(); place != net.generatedInitials.end(); place ++)
  {
    Place * newPlace = new Place( **place );
    newPlace->id = (*place)->id;
    newPlace->preset.clear();
    newPlace->postset.clear();
    generatedInitials.insert( newPlace );
    ...
    if (net.realStartNode!=NULL) {

        Place * newPlace = new Place( *net.realStartNode );
        newPlace->id = (net.realStartNode)->id;
        newPlace->preset.clear();
        newPlace->postset.clear();
        realStartNode = newPlace;
    }
*/
...
Place *PetriNet::newPlace(string my_role, communication_type my_type)
{
  ...
  //cerr << "newPlace (" << p->id << "): " << my_role_with_suffix << endl;
...
void PetriNet::cutByRoles() {
    ...
    list<Transition*> removeTransitions;
    bool remove;
    bool foundExcluded;
    ...
    for (set<Transition*>::iterator t = T.begin(); t != T.end(); t++) {
        ...
        if (remove) {
            ...
            if (!hasAlpha)
              removeTransitions.push_back(*t);
        }
    }

    // now delete all those transitions
    for (list<Transition*>::iterator t = removeTransitions.begin(); t != removeTransitions.end(); t++) {
        removeTransition(*t);
    }
...
Transition *PetriNet::newTransition(string my_role)
{
  ...
  //cerr << "newTransition (" << t->id << "): " << my_role_with_suffix << endl;
 ...
 *
 */
Transition* PetriNet::mergeTransitions(Transition *t1, Transition *t2)
{
  if (t1 == t2)
    return t1;
...
void PetriNet::fixPlaceNames()
{
    for (set< Place * >::iterator place = P_in.begin(); place != P_in.end(); place ++)
    {
        string placeName = (*place)->nodeShortName();

        while ( placeName.find("#",0) != string::npos)
        {
            int deletePos;
            deletePos = placeName.find_first_of("#",0);
            placeName.erase(deletePos,1);
        }
        ...
        (*place)->history.pop_front();
        (*place)->history.push_front(placeName);
 ...
 *
 * \result  the place that results in the merge
 */
Place* PetriNet::mergePlaces(Place * & p1, Place * & p2)
{
  ...
  if (realStartNode == p1 || realStartNode == p2)
      realStartNode = p12;

  if (generatedInitials.find(p1) != generatedInitials.end())
  {
      generatedInitials.erase(p1);
      generatedInitials.insert(p12);
  }

  if (generatedInitials.find(p2) != generatedInitials.end())
  {
      generatedInitials.erase(p2);
      generatedInitials.insert(p12);
  }
  ...
  p12->roles = setUnion(p1->roles,p2->roles);
 ...
 * \result  the place that results in the merge
 */
Place* PetriNet::mergePlaces(string role1, string role2)
{
  ...
  return mergePlaces( p1, p2 );
}

Place* PetriNet::mergePlaces(Place * & p1, string role2)
{
  Place * p2 = findPlace(role2);
...
Place* PetriNet::mergePlaces(string role1, Place * & p2)
{
  Place * p1 = findPlace(role1);
 ...
 * \result  the place that results in the merge
 */
Place* PetriNet::mergePlaces(unsigned int id1, string role1, unsigned int id2, string role2)
{
  return mergePlaces(toString(id1) + role1, toString(id2) + role2);
}
...
/*!
 * \brief returns a set of all internal places
 *
 * \return  the set of all internal places
 */
set<Place *> PetriNet::getInternalPlaces() const {
  return P; // copy the set of pointers
}
...
void PetriNet::fixIBMNet()
{
    set<Place*> removeNodes;

    // collect all nodes that have to be removed

    //  remove isolated output places
    for (set< Place * >::iterator place = P_out.begin(); place != P_out.end(); place ++)
    {
        if ((*place)->postset.empty() && (*place)->preset.empty())
        {
            removeNodes.insert(*place);
        }
    }

    //  remove isolated input places
    for (set< Place * >::iterator place = P_in.begin(); place != P_in.end(); place ++)
    {
        if ((*place)->postset.empty() && (*place)->preset.empty())
        {
    ...
    //  remove isolated internal places
    for (set< Place * >::iterator place = P.begin(); place != P.end(); place ++)
    {
        if ((*place)->postset.empty() && !(*place)->isFinal && (*place)->type == INTERNAL)
        {
            removeNodes.insert(*place);
        }
    }

    //  remove non-marked internal places with an empty preset
    for (set< Place * >::iterator place = P.begin(); place != P.end(); place ++)
    {
        if ((*place)->preset.empty() && (*place)->type == INTERNAL && (*place)->tokens==0)
        {
            removeNodes.insert(*place);
        }
    }
    ...
    for (set< Place * >::iterator place = removeNodes.begin(); place != removeNodes.end(); place ++)
    {
...
/*!
 * second step of the transformation of open nets to closed nets that can be
 * used for soundness checking
 *
 * - adds a new transition that consumes from each interface output place and each
 *   final places of the net and produces on a new final place
 * - makes each interface output place internal
 * - adds a looping transition to the final place so that the final marking is
 *   not a deadlock of the net
 */

void PetriNet::mergeFinalStates()
{
    set<Place*> removeNodes;
    set<Node*> noMoreOutput;

    // add a new final place and two new final transitions
    Place* finalPlace = newPlace("trueFinalPlace",INTERNAL);
    // one transition to consume from all interface output places and final places
    Transition* finalTransition = newTransition("finalize");
    newArc(finalTransition, finalPlace, STANDARD, 1);
// <Dirk.F start>
    // and one transition to loop on the final place to make the final
    // marking a live lock
    Transition* finalLoopT = newTransition("finalLoop");
    newArc(finalPlace, finalLoopT, STANDARD, 1);
    newArc(finalLoopT, finalPlace, STANDARD, 1);
// <Dirk.F end>

    // make all final places (as found in the final marking) pre-places of the
    // finalizing transition, remove these places from the final marking
    for (set< Place * >::iterator place = P.begin(); place != P.end(); place ++)
    {
        if ((*place)->isFinal)
        {
            (*place)->isFinal = false;
            newArc((*place),finalTransition, STANDARD, 1);
        }
    }

    // make each interface output place a pre-palce of the finalizing transition
    // and make it internal (add a new place and remove the old)
    for (set< Place * >::iterator place = P_out.begin(); place != P_out.end(); place ++)
    {
        Place* finalout = newPlace(((*place)->nodeShortName() + "_out"),INTERNAL);

        for (set<Node*>::iterator t = (*place)->preset.begin(); t != (*place)->preset.end();t++) {
            newArc((*t),(finalout), STANDARD, 1);
            noMoreOutput.insert((*t));
        }

        for (set<Node*>::iterator t = (*place)->postset.begin(); t != (*place)->postset.end();t++) {
            newArc((*t),(finalout), STANDARD, 1);
        }

        newArc((finalout),finalTransition, STANDARD, 1);
        removeNodes.insert((*place));
    }
    ...
    // make the new final place 'final'
...
/*!
 *
 * first step in transforming the given open net into a closed net by making all
 * input interface places of the open net internal, initially marked places
 *
 */
unsigned int PetriNet::fix_lola_output()
{
    ...
    // make all input places of the open net internal, initially marked places
    for (set< Place * >::iterator place = P_in.begin(); place != P_in.end(); place ++)
    {
        result++;
        // add a new place
        Place* inputPlace = newPlace(((*place)->nodeShortName() + "_in"),INTERNAL);
        // with corresponding arcs (an input place has only post-transitions
        for (set<Node*>::iterator t = (*place)->postset.begin(); t != (*place)->postset.end();t++) {
            newArc(inputPlace,(*t), STANDARD, 1);
            noMoreInput.insert((*t));	// remove post-transition of an input-place
        }
        // mark the new place
        inputPlace->mark();
