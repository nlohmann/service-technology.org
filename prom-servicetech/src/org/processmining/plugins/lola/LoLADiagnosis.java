...
public class LoLADiagnosis implements HTMLToString {
	...
	public String toHTMLString(boolean includeHTMLTags) {
		...
		lolaNames
				.put(
						"STUBBORN",
						"partial order reduction using stubborn sets (<a href=\"http://www.springerlink.com/content/mtj3p6183xchedr9/\">more</a>)");
		lolaNames
				.put(
						"COVER",
						"infinite sequence abstraction using the coverability graph technique (<a href=\"http://www.springerlink.com/content/h75484731mh5826u/\">more</a>)");
		lolaNames.put("SMALLSTATE", "use light weight data structure for markings");
		lolaNames
				.put("PREDUCTION",
						"compress markings using P-invariants (<a href=\"http://www.springerlink.com/content/20v8eyakvde5e558/\">more</a>)");
			...
			if (unbounded) {
				...
				if (path != null) {
					...
					for (int i = 0; i < path.size(); ++i) {
						...
						buffer.append(warning(((Transition) LoLA_Executor.getNode(path.getValue(i + ".0", "")))
								.getLabel()));
					}
				}
				...
				if (cycle != null) {
					...
					for (int i = 0; i < cycle.size(); ++i) {
						...
						buffer.append(warning(((Transition) LoLA_Executor.getNode(cycle.getValue(i + ".0", "")))
								.getLabel()));
					}
					buffer.append(")*</li>");
				}
		...
		if (reversibility != null) {
			...
			if (reversibility) {
				...
				CTMarking m = LoLA_Executor.constructMarking(net, state);
				buffer.append("<li>the initial marking is not reachable from " + warning(m.toHTMLString(false))
						+ "</li>");
			}
			buffer.append("</ul>");
		}
