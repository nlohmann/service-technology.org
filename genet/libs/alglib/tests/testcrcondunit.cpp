...
bool testcrcond(bool silent)
{
    ...
    for(n = 1; n <= maxn; n++)
    {
        ...
        for(pass = 1; pass <= passcount; pass++)
        {
            ...
            if( v>=threshold50*erc1 )
            {
                q50(0) = q50(0)+double(1)/double(passcount);
            }
            if( v>=threshold90*erc1 )
            {
            ...
            if( v>=threshold50*ercinf )
            {
                q50(2) = q50(2)+double(1)/double(passcount);
            }
            if( v>=threshold90*ercinf )
            {
        ...
        for(i = 0; i <= 3; i++)
        {
            err50 = err50||q50(i)<0.50;
            err90 = err90||q90(i)<0.90;
        }
    }
...
bool testcrcondunit_test_silent()
{
    ...
    return result;
#else
    return _i_testcrcondunit_test_silent();
#endif
}
