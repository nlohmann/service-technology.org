...
typedef enum {LOW, TIME, PROPERTY, MESSAGE} place_type;
typedef enum {STANDARD, READ, RESET} arc_type;
...
class Arc
{
  public:
    ...
    Arc(Node *source, Node *target, arc_type type, std::string inscription);
};
...
class Transition: public Node
{
  public:
    ...
    Transition(std::string name, std::string role, std::string guard);
};
...
class Place: public Node
{
  public:
    ...
    place_type type;
    /// constructor which adds a first role to the history
    Place(std::string name, std::string role, place_type type);
};
...
{
  public:
    Place *newPlace(std::string name, std::string role);
    Place *newPlace(std::string name, std::string role, place_type type);
    Transition *newTransition(std::string name, std::string role);
    Transition *newTransition(std::string name, std::string role, std::string guard);
    Arc *newArc(Node *source, Node *target);
    Arc *newArc(Node *source, Node *target, std::string inscription);
    Arc *newArc(Node *source, Node *target, arc_type type);
    Arc *newArc(Node *source, Node *target, arc_type type, std::string inscription);
