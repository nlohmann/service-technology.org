...
int main(int argc, char* argv[]) {
  ...
  l1.enqueue(1); l1.enqueue(4);
  assert(isSorted(l1));
  
  LinkedList<int> l2 = LinkedList<int>();
  l2.enqueue(6); l2.push(5); l2.enqueue(7);
  assert(isSorted(l2));
  
  l1.append(l2); assert(isSorted(l1));
  int num = 0;
  ...
  for(int i = 1; l1.isValid(); i++, l1.next()) assert(i == l1.getCons()->getContent());
  
  LinkedList<int> l3 = l2.copy(); l3.append(l1);
