...
/*!
 * Crops the path and the extension of a filename, e.g. crops "/home/user/test.og" to "test".
 *
 * \param  filename  a filename that might contain a path prefix and a file extension
 *
 * \return the filename without path prefix and file extension
 */
std::string crop_filename(std::string filename) {
    ...
    if ( result.find_last_of("/") != string::npos )
        result = result.substr(result.find_last_of("/") + 1,
                               result.length() - result.find_last_of("/") - 1);

    // strip everything behind the last "."
