...
// <Dirk.F start> use coloring for places according to BOM
  ...
  // task transition
  if ( ( historyContainsSubString("task.")
      || historyContainsSubString("callToTask.")
      || historyContainsSubString("callToProcess.")
      || historyContainsSubString("callToTask.")
      || historyContainsSubString("callToService.")))
  {
    if ( historyContainsSubString(".inputCriterion.") )
      result += "fillcolor=khaki1";
    else if  ( historyContainsSubString(".outputCriterion.") )
      result += "fillcolor=khaki3";
    else
      result += "fillcolor=khaki2";
  }
  // split/merge/fork/join
  if ( ( historyContainsSubString(".fire") || historyContainsSubString(".activate") )
    && ( historyContainsSubString("fork.")
        || historyContainsSubString("decision.")
        || historyContainsSubString("merge.")
        || historyContainsSubString("join.") ) )
  {
...
// <Dirk.F start> use coloring for places according to BOM
  ...
  else if (historyContainsSubString("process.") && historyContainsSubString("input."))
      result += "fillcolor=orange";
  // process output places
  else if (historyContainsSubString("process.") && historyContainsSubString("output."))
      result += "fillcolor=yellow";
  // process start place
  else if (historyContainsSubString("startNode."))
      result += "fillcolor=forestgreen";
  // process end node
  else if (historyContainsSubString("endNode."))
      result += "fillcolor=grey80";
  // process stop node
  else if (historyContainsSubString("stopNode."))
      result += "fillcolor=grey20";
  // task activated/complete places
  else if (  ( historyContainsSubString(".used") )
          && ( historyContainsSubString("task.")
              || historyContainsSubString("callToTask.")
              || historyContainsSubString("callToProcess.")
              || historyContainsSubString("callToTask.")
              || historyContainsSubString("callToService.")) )
  ...
  else if ( historyContainsSubString(".activated")
          && ( historyContainsSubString("fork.")
              || historyContainsSubString("decision.")
              || historyContainsSubString("merge.")
              || historyContainsSubString("join.")) )
...
void PetriNet::output_owfn(ostream *output) const
{
      ...
      (*output) << "FINALCONDITION" << endl;
        ...
        for ( set<Place*>::iterator place = P.begin(); place != P.end();place++)
        {
            if ((*place)->isFinal)
            {
                ...
                globals::finalCondition += (*place)->nodeFullName() + " = 1";
                firstrun = false;
            }
        }
  ...
  */
  ...
  // either any combination of endNodes is marked and all other places are empty
  finalCondition += "(";
  bool first = true;
  for (set<Place *>::const_iterator p = P.begin(); p != P.end(); p++) {
    if (!(*p)->historyContainsSubString("endNode."))
      continue;

    if (!first)
      finalCondition += " AND ";

    finalCondition += "(" + toLoLAident((*p)->nodeFullName()) + " >= 0)";
    first = false;
  }
  if (!first)
    finalCondition += " AND ";
  finalCondition += "ALL_OTHER_PLACES_EMPTY";
  finalCondition += ")\n";

  // or one of the stop nodes is marked (and the other places can have any number of tokens)
  for (set<Place *>::const_iterator p = P.begin(); p != P.end(); p++) {
    if (!(*p)->historyContainsSubString("stopNode."))
      continue;
  ...
  for (set<Transition *>::iterator t = T.begin(); t != T.end(); t++)
  {
    ...
    (*output) << "TRANSITION " << toLoLAident((*t)->nodeFullName());
#else
    ...
    for (set<Node *>::iterator pre = (*t)->preset.begin(); pre != (*t)->preset.end(); count++, pre++)
    {
      ...
      (*output) << toLoLAident((*pre)->nodeFullName());
#else
      (*output) << (*pre)->nodeName();
#endif
    ...
    for (set<Node *>::iterator post = (*t)->postset.begin(); post != (*t)->postset.end(); count++, post++)
    {
      ...
      (*output) << toLoLAident((*post)->nodeFullName());
#else
