...
public class Action_Views_GenerateView implements IWorkbenchWindowActionDelegate {

	public static final String ID = "hub.top.uma.ui.view.GenerateView";
    ...
    try {
      ...
      LinkedList<String[]> traces = ViewGeneration2.readTraces(fileName_trace);
      
      HashSet<Short> usedIDs = new HashSet<Short>();
      for (String[] trace : traces) {
        for (String event : trace)
          usedIDs.add(sys.nameToID.get(event));
      }
      
      DNodeSetElement toRemove = new DNodeSetElement();
      for (DNode e : sys.fireableEvents) {
        if (!usedIDs.contains(e.id)) toRemove.add(e);
      }
      
      sys.fireableEvents.removeAll(toRemove);
      System.out.println(sys.fireableEvents);
      
      final DNodeBP build = Uma.initBuildPrefix_View(sys, bound);
      ...
      Job bpBuildJob = new Job("constructing view") 
      {
        ...
        protected IStatus run(IProgressMonitor monitor) {
            ...
            while (build.step() > 0);
            PetriNet net = NetSynthesis.foldToNet_byLabel(build);

            IPath targetPath_dot = new Path(selectedFile_system.getFullPath().toString()+".view_rendered.dot");
            IPath targetPath_bp = new Path(selectedFile_system.getFullPath().toString()+".view_bp.dot");
            ActionHelper.writeFile(targetPath_dot, net.toDot());
            ActionHelper.writeFile(targetPath_bp, build.toDot());
