...
bool testminlbfgs(bool silent)
{
    ...
    referror = rep.terminationtype<=0||fabs(x(0)-2)>0.001||fabs(x(1))>0.001||fabs(x(2)-2)>0.001;
    ...
    minlbfgsresults(state, x, rep);
    lin1error = rep.terminationtype<=0||fabs(x(0)/ap::pi()-ap::round(x(0)/ap::pi()))>0.001;
    ...
    minlbfgsresults(state, x, rep);
    lin2error = rep.terminationtype<=0||fabs(x(0))>0.001;
    ...
    for(n = 1; n <= 10; n++)
    {
        ...
        for(m = 1; m <= n; m++)
        {
            ...
            for(i = 0; i <= n-1; i++)
            {
                eqerror = eqerror||fabs(x(i)-xe(i))>0.001;
            }
        }
    }
    ...
    minlbfgsresults(state, x, rep);
    converror = converror||fabs(x(0)-log(double(2)))>0.05;
    converror = converror||fabs(x(1))>0.05;
    converror = converror||fabs(x(2)-log(double(2)))>0.05;
...
bool testlbfgs_test_silent()
{
    ...
    return result;
#else
    return _i_testlbfgs_test_silent();
#endif
}
