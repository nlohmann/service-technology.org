...
int main(int argc, char** argv) {
            ...
            for (size_t a = 0; a < chor->events().size(); ++a) {
                for (size_t b = 0; b < chor->events().size(); ++b) {
                    ...
                    if (chor->disables(*q, a, b)) {
                        status("event %s disables event %s in state %d", chor->events()[a].c_str(),
            ...
            for (size_t a = 0; a < chor->events().size(); ++a) {
                for (size_t b = 0; b < chor->events().size(); ++b) {
                    ...
                    if (chor->enables(*q, a, b)) {
                        status("event %s enables event %s in state %d", chor->events()[a].c_str(), chor->events()[b].c_str(), *q);
    ...
    if (!args_info.quiet_flag) {
        ...
        for (size_t i = 0; i < projected.size(); ++i) {
            std::string peer_filename(fprefix + "peer" + chor->collaboration()[i]->name() + ".sa");
            Output output(peer_filename, "peer automaton " + chor->collaboration()[i]->name());
